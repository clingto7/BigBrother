...
import { accessSync, appendFileSync, constants, existsSync, mkdirSync, readFileSync, realpathSync, renameSync, rmSync, statSync, } from "fs";
...
function makeSelfUpdateCommand(installStep, uninstallStep, options = {}) {
    if (!uninstallStep)
        return installStep;
...
function isDirectPackageArtifactSpec(updateSpec) {
...
function getDefaultUpdatePackageName(installedPackageName, updateSpec) {
    if (isDirectPackageArtifactSpec(updateSpec)) {
        return installedPackageName;
    }
    return updateSpec;
}
function getSelfUpdateCommandForMethod(method, installedPackageName, updateSpec = installedPackageName, npmCommand, updatePackageName = getDefaultUpdatePackageName(installedPackageName, updateSpec)) {
    ...
    switch (method) {
        ...
        case "homebrew":
            return undefined;
        case "pnpm":
            return makeSelfUpdateCommand(makeSelfUpdateCommandStep("pnpm", ["install", "-g", updateSpec]), updatePackageName === installedPackageName
                ? undefined
                : makeSelfUpdateCommandStep("pnpm", ["remove", "-g", installedPackageName]), { uninstallAfterInstall });
        case "yarn":
            return makeSelfUpdateCommand(makeSelfUpdateCommandStep("yarn", ["global", "add", updateSpec]), updatePackageName === installedPackageName
                ? undefined
                : makeSelfUpdateCommandStep("yarn", ["global", "remove", installedPackageName]), { uninstallAfterInstall });
        case "bun":
            return makeSelfUpdateCommand(makeSelfUpdateCommandStep("bun", ["install", "-g", updateSpec]), updatePackageName === installedPackageName
                ? undefined
                : makeSelfUpdateCommandStep("bun", ["uninstall", "-g", installedPackageName]), { uninstallAfterInstall });
        case "npm": {
...
export function getSelfUpdateCommand(packageName, npmCommand, updateSpec = packageName, updatePackageName = getDefaultUpdatePackageName(packageName, updateSpec)) {
...
const envPrefix = (piConfigName || "pi")
    .toUpperCase()
    .replace(/[^A-Z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "") || "PI";
...
export const CONFIG_DIR_NAME = pkg.piConfig?.configDir || ".prime/agent";
...
export function getAgentTracesLogPath() {
 ...
 */
export function getDaemonLogPath(socketPath) {
    const normalized = normalizeSocketPath(socketPath);
    const hash = createHash("sha256").update(normalized).digest("hex").slice(0, 8);
    return join(getLogsDir(), `${basename(normalized)}.${hash}.log`);
}
export function getDaemonUpdateRestartManifestPath(socketPath, agentDir = getAgentDir()) {
...
export function getLegacyDaemonUpdateRestartManifestPath(agentDir = getAgentDir()) {
 ...
 */
export function appendRotatingLog(logPath, message, maxBytes = MAX_LOG_BYTES) {
    try {
...
export function getCronJobsPath(agentDir = getAgentDir()) {
...
export function getSessionsDir(agentDir = getAgentDir()) {
...
export function getSessionDirEnvOverride() {
