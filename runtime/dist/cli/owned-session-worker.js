...
let closeOwnerWatch;
export function isOwnedSessionWorkerProcess(environment = process.env) {
...
function hasNonSessionOperation(args) {
...
function isStartupBenchmark(environment) {
    const value = environment.PI_STARTUP_BENCHMARK?.toLowerCase();
    return value === "1" || value === "true" || value === "yes";
}
export function classifyOwnedSessionWorkerInvocation(args, stdinIsTTY, environment = process.env) {
...
export function createOwnedWorkerLaunchSpec(args, executable = process.execPath, execArgs = process.execArgv, entrypoint = process.argv[1]) {
    return createCliSubprocessLaunchSpec(args, executable, execArgs, entrypoint);
}
function readOwnedRecoveryDescriptor(path) {
    try {
...
function writeOwnedRecoveryDescriptor(path, profile, session) {
    const descriptor = {
        version: 1,
        profile,
...
export function installOwnedSessionRecoveryTracking(runtime) {
    const path = process.env[OWNED_RECOVERY_DESCRIPTOR_ENV];
    const profile = process.env[OWNED_PROFILE_ENV];
    if (!path || !profile) {
        return;
    }
...
export function createRpcRecoveryArgs(args, sessionPath) {
    const recovered = [];
...
export async function runOwnedSessionWorkerFrontend(args, profile) {
    ...
    const failPendingRpcCommands = () => {
        for (const pending of pendingRpcCommands.values()) {
            process.stdout.write(serializeJsonLine({
    ...
    const reapWorkerResources = (workerPid) => {
        if (!workerPid) {
            return;
        }
...
export async function maybeRunOwnedSessionWorkerFrontend(args, forceLegacyFrontend = false) {
...
export function installOwnedSessionWorkerOwnerWatch() {
...
export function closeOwnedSessionWorkerOwnerWatch() {
    closeOwnerWatch?.();
}
//# sourceMappingURL=owned-session-worker.js.map