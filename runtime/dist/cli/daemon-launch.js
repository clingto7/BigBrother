...
import { isSessionSummaryBusy } from "../modes/daemon/daemon-session-list.js";
...
export function isDaemonSessionSummary(value) {
...
function logDaemonLaunch(message) {
...
export async function probeDaemonVersion(socketPath) {
    let client;
    ...
    try {
        ...
        const current = hello.protocol.version === DAEMON_PROTOCOL_VERSION &&
            hello.schemaId === DAEMON_SCHEMA_ID &&
            hello.appVersion === VERSION;
        if (!current) {
            logDaemonLaunch(`running daemon on ${socketPath} is stale: daemon v${hello.appVersion}/proto${hello.protocol.version}` +
...
export async function listActiveDaemonSessionSummaries(client, options = {}) {
    return (await queryActiveDaemonSessions(client, options)).sessions;
}
async function queryActiveDaemonSessions(client, options = {}) {
    ...
    const busyClientOwnedSessionCount = data.busyClientOwnedSessionCount;
    if (busyClientOwnedSessionCount !== undefined &&
...
export class StaleDaemonError extends Error {
    ...
    constructor(socketPath, hello) {
        ...
        super(`An incompatible Prime Agent daemon is running.\n\n${daemonIdentity}\n` +
...
function hasProcessIdentityExited(identity, verifyProcessStartId = true) {
    if (!identity) {
        return true;
    }
    try {
...
async function waitForDaemonGone(socketPath, timeoutMs = 5000, requireSocketCleanup = false, expectedIdentity) {
...
export async function shutdownConnectedDaemonAndWait(client, socketPath, timeoutMs = 5000, hello = client.hello) {
...
export async function shutdownDaemonAndWait(socketPath, timeoutMs = 5000) {
    const client = new DaemonClient(socketPath);
    try {
...
export function isSessionBusy(summary) {
    return isSessionSummaryBusy(summary);
}
export async function probeRunningDaemonSessions(socketPath) {
    const client = new DaemonClient(socketPath);
    try {
        await client.connect(1000);
    }
...
async function shutdownStaleDaemonIfNotBusy(socketPath) {
    const client = new DaemonClient(socketPath);
    let connected = false;
    ...
    logDaemonLaunch(`replacing stale daemon on ${socketPath} (idle): ${loadedSessionCount} loaded session(s) will reload`);
    return shutdownDaemonAndWait(socketPath);
}
async function ensureDaemonRunning(socketPath, spawnCwd) {
    ...
    const child = spawn(process.execPath, [...process.execArgv, entrypoint, "--mode", "daemon", "--daemon-socket", socketPath], {
    ...
    throwIfFailed();
    throw new Error(`Timed out waiting for daemon to start on ${socketPath}.${readDaemonLogTail(socketPath, logOffset)}`);
}
function currentDaemonLogSize(socketPath) {
    try {
...
function readDaemonLogTail(socketPath, offset) {
 ...
 */
export function ensureInteractiveDaemonRunning(socketPath, spawnCwd) {
...
function findFirstEarlyLaunchPositional(args) {
...
export function shouldStartDaemonEarly(args, startupBenchmark) {
    if (startupBenchmark) {
        return false;
    }
    ...
    const isHelpCommand = firstPositional?.value === "help" && isHelpCommandRequest(args.slice(firstPositional.index + 1));
    if (firstPositional &&
...
export function maybeStartDaemonEarly(args) {
    ...
    const rawSocketPath = socketIndex !== -1 && args[socketIndex + 1] ? args[socketIndex + 1] : defaultDaemonSocketPath();
