...
export const PUBLIC_COMMAND_NAMES = new Set(COMMAND_SPECS.filter((spec) => spec.path.length === 1).map((spec) => spec.path[0]));
export const REMOVED_COMMAND_NAMES = new Set(["app", "daemon", "install", "manage", "remove", "uninstall"]);
const TOP_LEVEL_OPTION_GROUPS = [
    {
        heading: "Run options",
        options: [
...
export function getCommandSpec(path) {
    return COMMAND_SPECS.find((spec) => spec.path.length === path.length && spec.path.every((segment, index) => segment === path[index]));
}
export function getChildCommandSpecs(path) {
    return COMMAND_SPECS.filter((spec) => spec.path.length === path.length + 1 && path.every((segment, index) => segment === spec.path[index]));
}
export function isHelpCommandRequest(path) {
...
export function findCommandSuggestion(input, candidates) {
    let closest;
...
export function formatCommandHelp(path) {
    ...
    if (children.length > 0) {
        ...
        sections.push("", "Commands:", ...children.map((child) => `  ${child.path.at(-1).padEnd(width)}  ${child.summary}`));
    }
