  ...
  "openrouter": {
    ...
    "deepseek/deepseek-v4-pro": {
      ...
      cost: {
        input: 0.589512,
        output: 1.179024,
        cacheRead: 0.049125999999999996,
        cacheWrite: 0
      },
      contextWindow: 1048576,
      maxTokens: 384e3
...
function supportsFastMode(model) {
  const eligibleId = model.id === "gpt-5.4" || model.id === "gpt-5.5" || model.id === "gpt-5.6" || model.id.startsWith("gpt-5.6-");
  return eligibleId && (model.provider === "openai-codex" && model.api === "openai-codex-responses" || model.provider === "openai" && model.api === "openai-responses");
}
function calculateCost(model, usage, overrides) {
  ...
  usage.cost.cacheWrite = (overrides?.cacheWrite ?? model.cost.cacheWrite) / 1e6 * usage.cacheWrite;
