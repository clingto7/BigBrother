...
} from "./chunk-NEAJOPUF.js";
import {
  buildCopilotDynamicHeaders,
  hasCopilotVisionInput,
  isCloudflareProvider,
  resolveCloudflareBaseUrl
} from "./chunk-2DXIEQO5.js";
import {
  OpenAI
} from "./chunk-ENTVCTJV.js";
import {
  headersToRecord
} from "./chunk-CL7XFWST.js";
import {
  parseStreamingJson
...
var REASONING_DETAILS_SIGNATURE_TYPE = "openai-completions.reasoning_details.v1";
...
function decodeReasoningDetails(signature) {
  if (!signature?.startsWith("{")) return void 0;
  try {
    const parsed = JSON.parse(signature);
    if (parsed.type !== REASONING_DETAILS_SIGNATURE_TYPE || !Array.isArray(parsed.details)) return void 0;
...
var streamOpenAICompletions = (model, context, options) => {
  ...
  (async () => {
    ...
    try {
      ...
      const cacheWriteCost = cacheControl && hasStandardAnthropicCachePricing(model) ? getAnthropicCacheWriteCost(model.cost.input, cacheControl.ttl === "1h" ? "1h" : "5m") : void 0;
      ...
      const reasoningDetailsByIndex = /* @__PURE__ */ new Map();
        ...
        if (choice.delta) {
          ...
          if (reasoningDetails && Array.isArray(reasoningDetails)) {
            for (const detail of reasoningDetails) {
              ...
              const explicitIndex = typeof detailRecord.index === "number" ? detailRecord.index : void 0;
              ...
              if (detailRecord.type === "reasoning.encrypted" && typeof detailRecord.id === "string" && detailRecord.data) {
...
var streamSimpleOpenAICompletions = (model, context, options) => {
  ...
  const reasoningSpecified = requestedReasoning !== void 0;
  const clampedReasoning = reasoningSpecified ? clampThinkingLevel(model, requestedReasoning) : void 0;
  const reasoningEffort = clampedReasoning === "off" ? void 0 : clampedReasoning;
  const toolChoice = options?.toolChoice;
  return streamOpenAICompletions(model, context, {
    ...base,
    reasoningEffort,
    reasoningEnabled: reasoningSpecified ? clampedReasoning !== "off" : void 0,
    toolChoice
  });
};
...
function buildParams(model, context, options, compat = getCompat(model), cacheRetention = resolveCacheRetention(options?.cacheRetention), cacheControl = getCompatCacheControl(compat, cacheRetention)) {
  ...
  const params = {
    ...
    messages,
    stream: true,
    prompt_cache_key: model.baseUrl.includes("api.openai.com") && cacheRetention !== "none" || cacheRetention === "long" && compat.supportsLongCacheRetention ? options?.sessionId : void 0,
  ...
  if (compat.thinkingFormat === "zai" && model.reasoning) {
    ...
    if (options?.reasoningEffort && compat.supportsReasoningEffort) {
      ...
      openRouterParams.reasoning = compat.supportsReasoningEffort ? { effort: model.thinkingLevelMap?.off ?? "none" } : { enabled: false };
    }
...
function convertMessages(model, context, compat) {
  ...
  for (let i = 0; i < transformedMessages.length; i++) {
    ...
    if (msg.role === "user") {
      ...
      const replayReasoningDetails = msg.content.filter(isThinkingContentBlock).flatMap((block) => decodeReasoningDetails(block.thinkingSignature) ?? []);
      if (replayReasoningDetails.length > 0) {
        assistantMsg.reasoning_details = replayReasoningDetails;
      }
      const nonEmptyThinkingBlocks = msg.content.filter(isThinkingContentBlock).filter((block) => decodeReasoningDetails(block.thinkingSignature) === void 0).filter((block) => block.thinking.trim().length > 0);
      if (nonEmptyThinkingBlocks.length > 0) {
        if (compat.requiresThinkingAsText) {
          ...
          const reasoningField = compat.requiresReasoningContentOnAssistantMessages ? "reasoning_content" : nonEmptyThinkingBlocks[0].thinkingSignature || void 0;
          if (reasoningField) {
...
function detectCompat(model) {
  ...
  const isNonStandard = provider === "cerebras" || baseUrl.includes("cerebras.ai") || provider === "xai" || baseUrl.includes("api.x.ai") || baseUrl.includes("chutes.ai") || baseUrl.includes("deepseek.com") || isZai || isMoonshot || provider === "opencode" || baseUrl.includes("opencode.ai") || isCloudflareWorkersAI || isCloudflareAiGateway || isPrimeInference;
  ...
  const cacheControlFormat = isAnthropicModel && (provider === "openrouter" || isPrimeInference) ? "anthropic" : void 0;
  return {
    ...
    thinkingFormat: isDeepSeek ? "deepseek" : isZai ? "zai" : provider === "openrouter" || baseUrl.includes("openrouter.ai") ? "openrouter" : "openai",
    openRouterRouting: {},
