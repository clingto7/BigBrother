...
function anthropicSseError(data, requestId) {
  let errorType;
  let detail;
  try {
    const parsed = parseJsonWithRepair(data);
...
async function* iterateAnthropicEvents(response, signal, requestId) {
  if (!response.body) {
...
var streamAnthropic = (model, context, options) => {
  ...
  (async () => {
    ...
    try {
      ...
      let cacheWriteCost = cacheControl && usesAnthropicCachePricing ? getAnthropicCacheWriteCost(model.cost.input, cacheControl.ttl === "1h" ? "1h" : "5m") : void 0;
      ...
      for await (const event of iterateAnthropicEvents(response, options?.signal, requestId)) {
        if (event.type === "message_start") {
          ...
          calculateCost(
            model,
            output.usage,
            cacheWriteCost === void 0 ? void 0 : { cacheWrite: cacheWriteCost }
          );
...
function isAlwaysOnAdaptiveThinkingModel(modelId) {
  return modelId.includes("fable-5") || modelId.includes("mythos-5") || modelId.includes("mythos-preview");
}
function supportsAdaptiveThinking(modelId) {
  return modelId.includes("opus-4-6") || modelId.includes("opus-4.6") || modelId.includes("opus-5") || modelId.includes("opus-4-7") || modelId.includes("opus-4.7") || modelId.includes("opus-4-8") || modelId.includes("opus-4.8") || modelId.includes("sonnet-4-6") || modelId.includes("sonnet-4.6") || modelId.includes("sonnet-5") || modelId.includes("fable-5") || modelId.includes("mythos-5") || modelId.includes("mythos-preview");
}
function mapThinkingLevelToEffort(model, level) {
  const effective = level ? clampThinkingLevel(model, level) : void 0;
  const mapped = effective ? model.thinkingLevelMap?.[effective] : void 0;
...
function buildParams(model, context, isOAuthToken2, options, cacheControl) {
  const params = {
    model: model.id,
    messages: convertMessages(context.messages, model, isOAuthToken2, cacheControl),
  ...
  if (options?.temperature !== void 0 && !options?.thinkingEnabled && !isAlwaysOnAdaptiveThinkingModel(model.id)) {
  ...
  if (model.reasoning) {
    if (options?.thinkingEnabled) {
      ...
      if (supportsAdaptiveThinking(model.id)) {
        ...
        if (options.effort) {
          params.output_config = options.effort === "xhigh" || options.effort === "max" ? { effort: options.effort } : { effort: options.effort };
        }
      } else {
        params.thinking = {
          type: "enabled",
...
function convertMessages(messages, model, isOAuthToken2, cacheControl) {
  const params = [];
