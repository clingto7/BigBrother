...
function sideQuestionPrompt(question, isFirstTurn) {
...
export function startSideQuestion(parent, id, question, onEvent, previousTurns = []) {
    ...
    const previousTurnMessages = previousTurns.flatMap((turn, index) => [
        {
            role: "user",
    ...
    const done = Promise.resolve()
        ...
        await Promise.resolve(emit(abortRequested ? "cancelled" : "error", abortRequested ? undefined : errorMessage)).catch(() => undefined);
    })
        .finally(unsubscribe);
    return {
        done,
