...
function heartbeatCatalogSignature(jobs) {
...
export class AgentCronJobStore {
    filePath;
    sessionArtifactMode;
    sessionArtifactFiles = new Map();
    heartbeatChangeListeners = new Set();
    constructor(filePath, sessionArtifactMode = false) {
    ...
    static forSessionArtifacts() {
        return new AgentCronJobStore(undefined, true);
    }
    onHeartbeatChange(listener) {
    ...
    rebindSessionJobs(input) {
        ...
        const jobs = this.readJobs().map((job) => {
            ...
            if (job.activeSessionId === input.activeSessionId &&
    ...
    getHeartbeat(activeSessionId) {
        return this.readJobs()
            .filter((job) => {
            return (job.activeSessionId === activeSessionId &&
    ...
    cancelRlmHeartbeatsForSession(activeSessionId, now = new Date()) {
        const cancelled = [];
        const jobs = this.readJobs().map((job) => {
            if (job.activeSessionId !== activeSessionId ||
    ...
    cancelJobsForSession(input, now = new Date()) {
        ...
        const cancelled = [];
        const jobs = this.readJobs().map((job) => {
            const matches = (input.activeSessionId !== undefined && job.activeSessionId === input.activeSessionId) ||
    ...
    recordRunResult(id, result) {
        ...
        const jobs = this.readJobs().map((job) => {
            ...
            const nextRunAt = job.schedule.kind === "cron"
    ...
    writeJobs(jobs) {
        ...
        if (this.sessionArtifactMode) {
            ...
            withCronJobsStateLocks(paths, () => {
                const currentBySessionId = new Map([...this.sessionArtifactFiles].map(([sessionId, path]) => [sessionId, readJobsState(path)]));
                const incomingById = new Map(jobs.map((job) => [job.id, job]));
                const mergedJobsBySessionId = new Map();
                for (const [sessionId, current] of currentBySessionId) {
                    ...
                    mergedJobsBySessionId.set(sessionId, mergeFreshJobs(retained, jobs.filter((job) => job.sessionId === sessionId)));
                }
                const sessionIdByJobId = new Map([...mergedJobsBySessionId].flatMap(([sessionId, sessionJobs]) => sessionJobs.map((job) => [job.id, sessionId])));
...
export function migrateLegacyCronJobsToSessionArtifacts(filePath, options = {}) {
    ...
    const jobsByArtifact = new Map();
    for (const job of jobs) {
        const artifactPath = join(getSessionArtifactPathForFile(resolve(job.sessionFile), job.sessionId), SESSION_SCHEDULED_JOBS_FILENAME);
...
export class AgentCronScheduler {
    ...
    async runDue(now = this.now()) {
        ...
        try {
            ...
            for (const claimed of dispatches)
                claimed.endDispatch?.();
            if (claimedDispatches) {
                this.store.recoverInterruptedDispatchesById(claimedDispatches.map((dispatch) => dispatch.id), this.now());
            }
            throw error;
        }
        finally {
        ...
        const results = await Promise.all(dispatches.map(({ dispatch, endDispatch }) => this.queueDispatch(dispatch, endDispatch)));
    ...
    queueDispatch(dispatch, endDispatch) {
        ...
        const lane = task.then(() => undefined, () => undefined);
...
export function parseAgentCronSchedule(input, now = new Date()) {
...
export function resolveHeartbeatStreamingBehavior(deliveryMode) {
    return (deliveryMode ?? DEFAULT_HEARTBEAT_DELIVERY_MODE) === "follow_up" ? "followUp" : "steer";
}
export function parseHeartbeatCommand(input) {
...
export function formatAgentCronJob(job) {
...
function consumeDeliveryOption(text) {
    let rest = text.trim();
...
function consumeLeadingDeliveryFlag(text) {
...
function consumeTrailingDeliveryFlag(text) {
...
function parseDeliveryModeToken(token) {
...
function consumeEveryOption(text) {
    const match = /^--every(?:=|\s+)(?:"([^"]+)"|'([^']+)'|(\d+\s*(?:s|sec|secs|second|seconds|m|min|mins|minute|minutes|h|hr|hrs|hour|hours))|(\S+))(?:\s+|$)([\s\S]*)$/i.exec(text);
    if (!match) {
        return undefined;
    }
    return {
...
function consumeLeadingEverySchedule(text) {
    const match = /^(every|each)\s+\d+\s*(?:s|sec|secs|second|seconds|m|min|mins|minute|minutes|h|hr|hrs|hour|hours)\b/i.exec(text);
    if (!match) {
        return undefined;
    }
    return {
...
export function isHeartbeatCronJob(job) {
    return job.source === "heartbeat" || job.source === "rlm_heartbeat";
}
export function shouldDeferHeartbeatCronJob(job, activity) {
    ...
    const busyBesidesStreaming = activity.isCompacting === true ||
...
function withCronJobsStateLocks(paths, action) {
    const releases = [];
    try {
        for (const path of [...new Set(paths)].sort()) {
            ...
            for (let attempt = 0; attempt < 100; attempt++) {
                try {
                    ...
                    if (error.code !== "ELOCKED" || attempt === 99) {
                        throw error;
                    }
...
function writeJobsFile(path, jobs, mergeCurrent) {
...
function writeJobsState(path, state) {
...
function claimDueInState(state, dueAt, claimedAt) {
    const dispatches = [];
...
function recoverInterruptedInState(state, now, recovered, dispatchIds) {
...
function mergeFreshJobs(currentJobs, nextJobs) {
...
function isAtLeastAsFresh(candidate, current) {
...
function compareOptionalIso(left, right) {
    if (left === right) {
        return 0;
    }
    if (left === undefined) {
...
function normalizeOptionalLabel(label) {
...
function isAgentCronDispatchRecord(value) {
    if (!value || typeof value !== "object") {
        return false;
    }
    const candidate = value;
