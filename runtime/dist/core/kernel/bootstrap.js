...
let inFlightEnsureKernelPython = null;
...
function normalizePythonSkills(pythonSkills) {
    const byKey = new Map();
...
function readPythonSkillProjectName(skill) {
...
function parseDependencyPackageName(dependency) {
...
function findTomlArrayEnd(text, startIndex) {
    let inQuote;
    let escaped = false;
...
function readPythonSkillDependencyNames(skill) {
    const projectSection = readTomlProjectSection(skill.pyprojectPath);
    if (!projectSection) {
        return new Set();
    }
...
function resolveSiblingPythonSkillDependency(skill, dependencyName) {
...
function sortPythonSkillsForInstall(pythonSkills) {
    ...
    for (const skill of pythonSkills) {
        dependenciesBySkill.set(skill, [...readPythonSkillDependencyNames(skill)]
            .map((dependencyName) => byProjectName.get(dependencyName) ?? resolveSiblingPythonSkillDependency(skill, dependencyName))
...
function formatPythonSkillInstallArgs(skill) {
    return ["--editable", skill.packagePath];
}
function ensureKernelPythonKey(pythonSkills) {
    return [
...
export function getKernelVenvDir() {
...
async function resolveWritableKernelVenvDir() {
    ...
    try {
        ...
        try {
            ...
            throw new Error(`couldn't create kernel venv directory at ${primary} or ${fallback}; set PRIME_AGENT_KERNEL_PYTHON to a python with a current prime-agent-runtime installed. ${errorMessage(fallbackError)}`);
        }
    }
}
...
async function pythonImports(python, moduleName) {
    try {
...
async function hasPrimeAgentRuntime(python) {
    try {
...
async function missingRlmExtraImportLabels(python) {
    const missing = [];
...
async function missingPythonSkillImportLabels(python, pythonSkills) {
    const missing = [];
    for (const skill of pythonSkills) {
...
function bootstrapLockDir(venv) {
...
async function lockMissingPidIsStale(lockDir) {
    try {
...
async function acquireBootstrapLock(venv) {
...
async function ensureUv(options) {
    ...
    if (await isExecutable(localUv))
        return localUv;
    const shouldInstallUv = process.env.PRIME_AGENT_INSTALL_UV === "1" || (!options.onProgress && (await confirmUvInstall()));
    if (!shouldInstallUv) {
        throw new Error(`uv is required to set up the Python kernel. Install uv yourself: ${UV_INSTALL_COMMAND}, ` +
    ...
    try {
        ...
        throw new Error(`couldn't install uv from astral.sh; install it yourself: ${UV_INSTALL_COMMAND}, then re-run prime-agent. ${errorMessage(error)}`);
    }
    if (await isExecutable(localUv))
        return localUv;
    const installedFromPath = await findExecutable("uv");
    if (installedFromPath)
        return installedFromPath;
    throw new Error("uv install completed but binary not found at ~/.local/bin/uv");
}
async function confirmUvInstall() {
    if (process.env.PRIME_AGENT_INSTALL_UV === "0")
        return false;
...
async function readBootstrapVersion(venv) {
    try {
        const raw = await readFile(path.join(venv, BOOTSTRAP_VERSION_FILE), "utf8");
        const parsed = JSON.parse(raw);
        if (!isRecord(parsed) || typeof parsed.schema !== "number")
            return null;
        const extraUvArgs = Array.isArray(parsed.extraUvArgs) &&
            parsed.extraUvArgs.every((v) => typeof v === "string")
            ? parsed.extraUvArgs
            : undefined;
        let pythonSkills;
        if (Array.isArray(parsed.pythonSkills)) {
            if (!parsed.pythonSkills.every((v) => {
                if (!isRecord(v))
                    return false;
                return (typeof v.importName === "string" &&
            ...
            pythonSkills = parsed.pythonSkills;
        }
        return {
...
function bootstrapVersionCurrent(version, runtimeIdentity, pythonSkills) {
    return (version !== null &&
        bootstrapBaseVersionCurrent(version, runtimeIdentity) &&
        pythonSkillsMatch(version.pythonSkills, pythonSkills));
}
function bootstrapBaseVersionCurrent(version, runtimeIdentity) {
    return (version?.schema === BOOTSTRAP_SCHEMA &&
...
async function writeBootstrapVersion(venv, runtimeIdentity, pythonSkills) {
    const version = {
...
async function resolveRuntimeSourceDir() {
...
async function hashRuntimeSource(sourceDir) {
    const rlmDir = path.join(sourceDir, "src", "rlm");
    const files = [path.join(sourceDir, "pyproject.toml")];
...
async function syncPythonSkills(uv, venv, python, runtimeIdentity, pythonSkills, options) {
    const version = await readBootstrapVersion(venv);
    const installedPythonSkills = [];
    const currentPythonSkills = new Map((version?.pythonSkills ?? []).map((skill) => [`${skill.importName}\0${skill.packagePath}`, skill]));
    const pythonSkillsByProjectName = new Map(pythonSkills.map((skill) => [readPythonSkillProjectName(skill).replaceAll("_", "-").toLowerCase(), skill]));
    const dependenciesBySkill = new Map(pythonSkills.map((skill) => [
        skill,
        [...readPythonSkillDependencyNames(skill)]
            .map((dependencyName) => pythonSkillsByProjectName.get(dependencyName) ??
    ...
    for (const skill of sortPythonSkillsForInstall(pythonSkills)) {
        ...
        const localDependencyArgs = localDependencies
            ...
            const installedThisSync = installedPythonSkills.some((installed) => installed.importName === dependency.importName &&
        ...
        try {
            ...
            installedPythonSkills.push(skill, ...localDependencies.filter((dependency) => !installedPythonSkills.includes(dependency)));
        }
        catch (error) {
            reportProgress(options, `Warning: Python skill ${skill.importName} failed to install and will be unavailable: ${errorMessage(error)}`);
        }
    }
    await writeBootstrapVersion(venv, runtimeIdentity, installedPythonSkills);
}
async function kernelBaseReady(python, venv, runtimeIdentity) {
    return ((await hasPrimeAgentRuntime(python)) &&
        bootstrapBaseVersionCurrent(await readBootstrapVersion(venv), runtimeIdentity));
}
async function kernelReady(python, venv, runtimeIdentity, pythonSkills) {
    return ((await hasPrimeAgentRuntime(python)) &&
...
function formatBootstrapFailure(error) {
    return new Error(`Failed to set up the Python kernel runtime. ${errorMessage(error)}\n` +
...
async function ensureKernelPythonUncached(options, pythonSkills) {
    ...
    if (override) {
        ...
        if (!(await hasPrimeAgentRuntime(python))) {
            missing.push("a current prime-agent-runtime with callable rlm.run, rlm.host_request, and explicit harness CRUD methods");
        }
        if (missing.length === 0) {
        ...
        if (missing.length === 0 && pythonSkills.length > 0) {
            ...
            if (missingPythonSkills.length > 0) {
                reportProgress(options, `Warning: Python skills unavailable in PRIME_AGENT_KERNEL_PYTHON and will be disabled: ${missingPythonSkills.join(", ")}`);
            }
        }
        if (missing.length === 0)
            return python;
...
export function ensureKernelPython(options = {}) {
    const pythonSkills = normalizePythonSkills(options.pythonSkills);
    const key = ensureKernelPythonKey(pythonSkills);
    if (inFlightEnsureKernelPython?.key === key)
        return inFlightEnsureKernelPython.promise;
    const promise = ensureKernelPythonUncached(options, pythonSkills).finally(() => {
        if (inFlightEnsureKernelPython?.promise === promise)
            inFlightEnsureKernelPython = null;
    });
