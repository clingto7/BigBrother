...
export class ReplKernelManager {
    ...
    activeExecution;
    activeExecutionIdleWaiters = new Set();
    lateSentAgentMessageHandlers = new Map();
    /** Resolvers for done events outside the active execution (the shutdown reply). */
    pendingDoneWaiters = new Map();
    ...
    pendingBackgroundOutputTruncated = false;
    inFlightHostRequests = new Set();
    state = "idle";
    ...
    async doStart(startOptions) {
        ...
        try {
            ...
            const canRetryStartup = this.state !== "shutdown";
    ...
    async waitForReady(child) {
        ...
        if (!ready)
            throw new Error("Kernel ready state is missing");
        let timeout;
        let onExit;
        try {
            return await new Promise((resolve, reject) => {
                ...
                timeout = globalThis.setTimeout(() => {
                    const tail = this.kernelStderr.slice(-1024);
                    reject(new Error(`Kernel did not become ready within ${READY_TIMEOUT_MS}ms. stderr tail:\n${tail || "(empty)"}`));
                }, READY_TIMEOUT_MS);
                timeout.unref?.();
            });
        }
    ...
    handleEvent(event) {
        ...
        if (type === "host_request") {
            if (typeof event.id === "string")
                this.startHostRequest(event.id, event.data);
            return;
        }
    ...
    async enqueueExecute(code, opts, executionTimeoutMs) {
    ...
    async enqueueRequest(requestFields, code, opts, executionTimeoutMs) {
    ...
    async executeInner(requestFields, code, opts, started) {
    ...
    finishActiveExecution(execution) {
        if (this.activeExecution !== execution) {
            return;
        }
        this.resolveExecution(execution, { clearActive: true });
    }
    resolveExecution(execution, options) {
        ...
        if (!execution.settled) {
            ...
            if (execution.opts.signal?.aborted)
                status = "aborted";
    ...
    dispatchLateSentAgentMessage(requestId, value) {
    ...
    registerLateSentAgentMessageHandler(requestId, handler) {
        this.lateSentAgentMessageHandlers.set(requestId, handler);
    ...
    async waitForActiveExecutionToClearForReuse(signal) {
        const started = Date.now();
        while (this.activeExecution && Date.now() - started < KERNEL_BUSY_REUSE_WAIT_MS) {
            ...
            const cleared = await this.waitForActiveExecutionToClear(signal, Math.max(1, Math.min(KERNEL_BUSY_INTERRUPT_INTERVAL_MS, remaining)));
    ...
    startHostRequest(requestId, data) {
        ...
        const task = (async () => {
            try {
                ...
                try {
                    ...
                    this.appendKernelDiagnostic(`failed to send host request error reply for ${requestId}: ${errorMessage(replyError)}`);
                }
            }
        })();
    ...
    cleanupResources(killSignal = "SIGTERM") {
        ...
        if (child) {
            ...
            if (pid !== undefined)
                reapKernelOrphanProcesses(pid);
        }
        this.startPromise = undefined;
    }
    async waitForKernelExit() {
        const child = this.child;
        if (!child)
            return;
        if (child.exitCode !== null || child.signalCode !== null)
    ...
    async waitForHostRequestsToSettle(tasks, timeoutMs) {
        let timeout;
        ...
        const result = await Promise.race([Promise.allSettled(tasks).then(() => "settled"), timeoutPromise]);
        if (timeout) {
            globalThis.clearTimeout(timeout);
        }
        if (result === "timeout") {
    ...
    async performShutdown(opts) {
        ...
        try {
            ...
            this.appendKernelDiagnostic(`graceful shutdown failed (killing instead): ${error instanceof Error ? error.message : String(error)}`);
        }
        finally {
            if (shutdownTimer)
    ...
    async captureSnapshot(options = {}) {
        ...
        try {
            ...
            }, "", { internal: true }, options.executionTimeoutMs);
            if (r.status !== "ok" || !r.doneFields) {
                this.appendKernelDiagnostic(`state snapshot ${r.status === "aborted" ? "timed out" : "failed"}: ${r.error?.evalue ?? r.stderr}`);
                return null;
            }
    ...
    async listNamespaceNames(signal) {
        if (!this.isRunning)
            return null;
        try {
