 ...
 */
import { getLogger, modelsAreEqual } from "@earendil-works/pi-ai";
...
function findPreferredDefaultModel(availableModels) {
    const primeInferenceDefault = availableModels.find((model) => model.provider === "prime-inference" && model.id === PRIME_INFERENCE_DEFAULT_MODEL_ID);
 ...
 */
export function resolveModelScopeFromModels(patterns, availableModels) {
    const scopedModels = [];
...
export async function findInitialModel(options) {
    ...
    if (cliProvider && cliModel) {
        ...
        if (resolvedModel) {
            if (isPrivatePrimeInferenceModel(resolvedModel)) {
                const availableModel = (await getAvailableModels()).find((candidate) => modelsAreEqual(candidate, resolvedModel));
                if (!availableModel) {
    ...
    if (defaultProvider && defaultModelId) {
        ...
        const found = availableModels.find((candidate) => candidate.provider === defaultProvider && candidate.id === defaultModelId) ??
...
export async function restoreModelFromSession(savedProvider, savedModelId, currentModel, shouldPrintMessages, modelRegistry) {
    const availableModels = await modelRegistry.refreshAvailableModels();
    const restoredModel = availableModels.find((candidate) => candidate.provider === savedProvider && candidate.id === savedModelId);
    if (restoredModel) {
