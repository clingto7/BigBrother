...
function defaultPrimeCliConfigPath() {
...
function readPrimeCliConfigData(configPath) {
    let data = {};
...
function writePrimeCliConfigData(configPath, data) {
    const dir = dirname(configPath);
    if (!existsSync(dir)) {
        mkdirSync(dir, { recursive: true, mode: 0o700 });
    }
    const tempPath = join(dir, `.${basename(configPath)}.${process.pid}.${Date.now()}.${Math.random().toString(36).slice(2)}.tmp`);
    let fd = openSync(tempPath, "wx", 0o600);
    try {
...
function clearPrimeTeamFields(data) {
    delete data.team_id;
    delete data.team_name;
    delete data.team_role;
}
export function loadPrimeCliConfig(configPath = defaultPrimeCliConfigPath()) {
...
export function savePrimeCliApiKey(apiKey, configPath = defaultPrimeCliConfigPath()) {
    const data = readPrimeCliConfigData(configPath);
    data.api_key = apiKey;
...
export function clearPrimeCliCredentials(configPath = defaultPrimeCliConfigPath()) {
    const data = readPrimeCliConfigData(configPath);
    delete data.api_key;
    clearPrimeTeamFields(data);
    writePrimeCliConfigData(configPath, data);
...
export function savePrimeCliTeamSelection(team, configPath = defaultPrimeCliConfigPath()) {
    const data = readPrimeCliConfigData(configPath);
    if (team) {
...
export function resolvePrimeAgentTracesBaseUrl(baseUrl) {
    return normalizeBaseUrl(baseUrl ?? stringEnv("PRIME_AGENT_TRACES_BASE_URL"));
}
function resolvePrimeAgentTracesChallengeConfig(config) {
    return {
...
async function readJsonObject(response, context) {
    let parsed;
    try {
...
export async function fetchPrimeTeams(apiKey, baseUrl, options = {}) {
    ...
    while (true) {
        ...
        }, requestTimeoutMs, options.signal);
        if (!response.ok) {
...
function createPrimeChallengeKeypair() {
...
function decryptPrimeChallengeResult(privateKey, encryptedResult) {
...
async function generatePrimeChallenge(config, publicKey, fetchFn, timeoutMs, signal) {
    const response = await fetchWithTimeout(fetchFn, `${config.baseUrl}/api/v1/auth_challenge/generate`, {
        method: "POST",
...
async function pollPrimeChallengeResult(config, challenge, privateKey, fetchFn, timeoutMs, pollIntervalMs, signal) {
    while (true) {
        throwIfCancelled(signal);
...
async function runPrimeBrowserLogin(config, callbacks, fetchFn, timeoutMs, pollIntervalMs, scope) {
...
async function checkPrimeScopeAccess(apiKey, baseUrl, scopeName, scopeLabel, options = {}) {
    const fetchFn = options.fetchFn ?? fetch;
    const requestTimeoutMs = options.requestTimeoutMs ?? DEFAULT_REQUEST_TIMEOUT_MS;
...
export async function checkPrimeInferenceAccess(apiKey, baseUrl, options = {}) {
    return checkPrimeScopeAccess(apiKey, baseUrl, "inference", "inference", options);
}
export async function checkPrimeAgentTracesAccess(apiKey, baseUrl, options = {}) {
    return checkPrimeScopeAccess(apiKey, baseUrl, "agent_traces", "agent trace", options);
}
function formatAccessFailure(result) {
    const status = result.status === undefined ? "" : `HTTP ${result.status}: `;
    return `${status}${result.message}`;
}
export async function loginPrimeInference(callbacks, options = {}) {
    ...
    if (config.apiKey) {
        ...
        callbacks.onProgress?.(`Existing Prime CLI key cannot access Prime Inference (${formatAccessFailure(access)}). Starting browser login...`);
    }
    else {
...
export async function loginPrimeAgentTraces(callbacks, options = {}) {
    const config = loadPrimeCliConfig(options.configPath);
    ...
    if (config.apiKey) {
        ...
        callbacks.onProgress?.(`Existing Prime CLI key cannot upload Prime Agent traces (${formatAccessFailure(access)}). Starting browser login...`);
    }
    else {
        callbacks.onProgress?.("No Prime CLI API key found. Starting browser login...");
    }
    const apiKey = await runPrimeBrowserLogin(traceConfig, callbacks, fetchFn, requestTimeoutMs, pollIntervalMs, "agent_traces");
    throwIfCancelled(callbacks.signal);
...
//# sourceMappingURL=prime-inference-auth.js.map