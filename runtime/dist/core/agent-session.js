...
import { Agent, AgentContinueError, } from "@earendil-works/pi-agent-core";
import { clampThinkingLevel, cleanupSessionResources, getSupportedThinkingLevels, isContextOverflow, modelsAreEqual, resetApiProviders, supportsFastMode, } from "@earendil-works/pi-ai";
...
import { COMPACT_SKILL_NAME, calculateContextTokens, collectEntriesForBranchSummary, compact, estimateContextTokens, generateBranchSummary, prepareCompaction, serializeConversation, shouldCompact, } from "./compaction/index.js";
import { computeOwnAndTotalUsage, loadContextTreeChildFromDisk, loadContextTreeChildrenFromDisk, } from "./context-tree.js";
...
import { createGoalContextMessage, emptyGoalState, GOAL_CONTEXT_CUSTOM_TYPE, GOAL_CONTEXT_PREVIEW_LABEL, GOAL_SKILL_NAME, GOAL_STATE_CUSTOM_TYPE, goalHostResponse, goalTokenDeltaForUsage, isPersistedGoalState, normalizeGoalState, validateGoalBudget, validateGoalObjective, } from "./goals.js";
import { snapshotPathIn } from "./kernel/state-snapshot.js";
import { convertToLlm, createCompactionOutcomeMessage, createHeartbeatPromptMessage, createRefinementOutcomeMessage, createRlmChildFailureMessage, createRlmChildTerminalNoticeMessage, createSessionSlashCommandMessage, createSessionSlashCommandResultMessage, HEARTBEAT_PROMPT_CUSTOM_TYPE, HEARTBEAT_PROMPT_PREVIEW_LABEL, IPYTHON_STATE_RESTORED_CUSTOM_TYPE, isSessionSlashCommandMessage, RLM_CHILD_FAILURE_CUSTOM_TYPE, RLM_CHILD_TERMINAL_NOTICE_CUSTOM_TYPE, } from "./messages.js";
import { throwIfPromptAdmissionCancelled } from "./prompt-admission.js";
import { expandPromptTemplate } from "./prompt-templates.js";
import { appendGlobalRefinement, applyRefinementProposal, generateRefinementId, getGlobalHarnessStateDir, getLocalHarnessStateDir, getRefinementHistory, inferRefinementResultScope, loadGlobalRefinementHistory, loadHarnessState, mergeHarnessStates, mergeRefinementHistory, normalizeRefinementProposal, planRefinement, REFINE_SKILL_NAME, reviewAutoRefine, saveHarnessState, } from "./refinement/index.js";
import { resolveConfigValue } from "./resolve-config-value.js";
import { createDefaultRlmSubagentSessionName, createRlmDeleteSubagentHostHandler, createRlmFindModelsHostHandler, createRlmListSubagentsHostHandler, createRlmRunHostHandler, findRlmModelMatches, normalizeRequestedRlmSubagentModel, normalizeRequestedRlmSubagentSessionName, normalizeRequestedRlmSubagentThinkingLevel, } from "./rlm-runtime.js";
import { ActionStore, canSelectSessionAction, queuedMessageLaneDeliveryPolicy, transitionSessionAction, } from "./session-action-store.js";
...
import { parseRefineCommandOptions, parseSessionSlashCommand, parseSlashCommand, } from "./slash-commands.js";
...
export { parseSkillBlock } from "./skill-blocks.js";
...
function turnExecutionPoliciesEqual(left, right) {
    return (left.preparation.initialRefineBarrier === right.preparation.initialRefineBarrier &&
...
function oncePreflight(preflightResult) {
    let settled = false;
...
function cloneCustomMessage(message) {
    return {
        ...message,
        content: Array.isArray(message.content) ? message.content.map((block) => ({ ...block })) : message.content,
    };
}
function cloneQueuedAgentMessage(message) {
    if (message.role === "custom")
        return cloneCustomMessage(message);
    return {
        ...message,
        content: Array.isArray(message.content) ? message.content.map((block) => ({ ...block })) : message.content,
    };
}
function primaryDeliveryRecord(action) {
    if (action.payload.kind !== "turn")
        throw new Error(`Session action ${action.id} is not a turn`);
    const record = action.payload.records.find((candidate) => candidate.role === "primary");
    if (!record)
        throw new Error(`Turn action ${action.id} has no primary delivery record`);
    return record;
}
...
function queuedAgentMessagePreview(action) {
...
function visibleSessionActionProjection(actions) {
    return actions.filter((action) => action.payload.kind === "session_command" ||
...
function parsePersistedIpythonSentAgentMessage(value) {
...
function appendSentAgentMessageToToolResult(message, toolCallId, sentMessage) {
...
function injectedMessagePreviewLabel(message) {
...
function createAgentMessageDeferred() {
    const deferred = {};
...
function createPostCompactionContinuationSettlement() {
    return { ...createAgentMessageDeferred(), continueAfterSessionInput: false, settled: false };
}
const KERNEL_STATE_LISTING_TIMEOUT_MS = 5000;
const RLM_MAX_DEPTH_STATE_CUSTOM_TYPE = "rlm_max_depth_state";
function noopRlmChildAbort() { }
function noopRlmChildEventUnsubscribe() { }
function autoRefineInstructions(reason, review) {
...
function parseDepth(value, fallback, name) {
...
function isPersistedRlmMaxDepthState(value) {
    return (typeof value === "object" && value !== null && isNonNegativeInteger(value.maxDepth));
}
function parseGoalBudgetValue(value) {
    if (!/^[1-9]\d*$/.test(value)) {
...
export function compactRlmText(text, maxLength = 160) {
...
export function rlmChildLabel(prompt) {
...
function waitForPromiseOrAbort(promise, signal, abortMessage) {
    if (!signal)
        return promise;
    if (signal.aborted)
...
function attributeChildUsage(parentUsage, childUsage) {
    const parentContextTokens = parentUsage.totalTokens ||
...
export class AgentSession {
    ...
    sessionManager;
    settingsManager;
    _serviceTierPreference;
    _scopedModels;
    _unsubscribeAgent;
    _eventListeners = [];
    _lastSessionActionSnapshot = {
        queuedCount: 0,
        steering: [],
        followUps: [],
    };
    ...
    _sessionInputPump = Promise.resolve();
    ...
    _sessionInputSuspendedForUpdateRestart = false;
    ...
    _durableRlmTerminalNoticeActionIds = new Set();
    _sessionActionCommitTail = Promise.resolve();
    _sessionActionCommitOwner;
    _pendingSessionActionFenceWaiters = 0;
    _sessionActionCommitContext = new AsyncLocalStorage();
    _sessionActionCommitDisposeAbortController = new AbortController();
    // Checkpoint, handoff, and activity waiters share lifecycle-edge notifications to avoid polling.
    _sessionInputCheckpointWaiters = new Set();
    _pendingNextTurnMessages = [];
    _goalState = emptyGoalState();
    _goalAccountingStartedAt = undefined;
    _goalContinuationAwaitsRlmWork = false;
    _goalAccountedAssistantMessages = new WeakSet();
    _goalAbortInProgress = false;
    _autonomousState;
    _autonomousContinuationSuppressionDepth = 0;
    _autonomousContinuationSuppressedMessages = new WeakSet();
    _compactionAbortController = undefined;
    _autoCompactionAbortController = undefined;
    _compactionOperation = undefined;
    ...
    _branchSummaryOperation = undefined;
    ...
    _execEnvProvider;
    _turnIndex = 0;
    _modelSelectEmitQueue = Promise.resolve();
    _modelSelectEmitQueueIdle = true;
    ...
    _allowedToolNames;
    _includeGoals;
    _includeCompactSkill;
    _rlmHeartbeatController;
    _agentMessageController;
    _agentObserveController;
    _mcpManager;
    _baseToolsOverride;
    _sessionStartEvent;
    _extensionUIContext;
    ...
    _pendingRlmSubagentSessionNames = new Set();
    ...
    _autoRefineOperations = new Set();
    _scheduledAutoRefineTimers = new Set();
    _compactAutoRefinePending = false;
    _turnIntervalAutoRefinePending = false;
    _postCompactionContinuationScheduled = false;
    _postCompactionContinuationSettlement;
    _postCompactionContinuationMessages = [];
    _scheduledPostCompactionContinuationMessages = [];
    _queuedAutonomousThresholdContinuations = new WeakMap();
    _queuedAutonomousContinuationSnapshots = new WeakMap();
    _pendingThresholdCompactionAutonomousMessages = [];
    _queuedGoalThresholdContinuation;
    _pendingAutoRefineReview;
    _autoRefineBranchVersion = 0;
    _autoRefineReviewAbort;
    _refineAbortController;
    _autoRefineReviewer;
    _serializedRefine;
    _refineInFlight;
    _refinePlanInFlight;
    _serializedPlanInFlight;
    _serializedPlanClaim;
     ...
     */
    setRlmHeartbeatController(controller) {
    ...
    replaceAcpMcpServers(servers, ownerId) {
        if (this.isStreaming)
    ...
    _rememberLateIpythonSentAgentMessage(toolCallId, message) {
    ...
    _applyLateIpythonSentAgentMessages(message) {
    ...
    _recordLateIpythonSentAgentMessage(toolCallId, message) {
        const record = () => {
    ...
    _goalWithCurrentWallClock(now = Date.now()) {
    ...
    _cancelSessionActions(predicate, error, candidates = this._actionStore.clearableActions()) {
        ...
        const preparing = this._actionStore
            .activeActions()
            .filter((action) => action.payload.kind === "turn" && action.lifecycle.state === "preparing");
        const previousAnchor = preparing.at(-1);
        const actions = this._actionStore.remove(predicate, candidates);
        const restorableMessages = [];
        ...
        for (const action of actions) {
            ...
            if (action.payload.kind === "turn") {
                ...
                const restorable = payload.records
                    .filter((record) => (record.role === "next_turn" || (payload.acceptedAgentMessage && record.role === "prefix")) &&
                ...
                if (dispatched) {
                    ...
                    this.agent.state.messages = this.agent.state.messages.filter((message) => !payload.captureRunMessages?.has(message));
                }
            }
            if (!dispatched) {
    ...
    _clearQueuedGoalContexts() {
        ...
        this._pendingNextTurnMessages = this._pendingNextTurnMessages.filter((message) => message.customType !== GOAL_CONTEXT_CUSTOM_TYPE);
        this.agent.removeQueuedMessages((message) => message.role === "custom" && message.customType === GOAL_CONTEXT_CUSTOM_TYPE);
        this._cancelSessionActions((action) => action.payload.kind === "turn" && action.payload.customMessage?.customType === GOAL_CONTEXT_CUSTOM_TYPE, new Error("Queued goal context was cleared before delivery."));
        this._emitQueueUpdate();
    }
    _startGoal(objectiveText, tokenBudget) {
    ...
    _pauseGoal(reason = "Paused by user") {
        this._clearQueuedGoalContexts();
        if (this._goalState.status !== "active") {
            this._emitGoalUpdate();
            return;
    ...
    async _resumeGoal() {
        ...
        const exhausted = this._goalState.tokenBudget !== undefined && this._goalState.tokensUsed >= this._goalState.tokenBudget;
        const nextStatus = exhausted ? "budget_limited" : "active";
        this._setGoalState({
            ...this._goalState,
    ...
    _finishGoalWithError(errorMessage) {
    ...
    _finishGoalForTerminalAssistantMessage(message) {
        if (this._goalState.status !== "active") {
            return;
        }
    ...
    _stopGoalContinuationForTerminalMessage(message) {
    ...
    _parseAutonomousSlashCommand(text) {
        const command = parseSessionSlashCommand(text);
    ...
    async _handleAutonomousSlashCommand(text) {
    ...
    async _validateCanStartAgentRun() {
        if (!this.model) {
     ...
     */
    _ensureGoalRuntimeActive(context) {
    ...
    _maybeResumeGoalContinuationAfterRlmWork() {
        if (!this._goalContinuationAwaitsRlmWork)
            return;
        if (this._disposed || this._disposing || this._hasUnsettledRlmQuiescenceWork())
            return;
        ...
        try {
            ...
            this._admitSessionInput(this._createPreparedTurnAction("followUp", normalized.text, normalized.images, {
                message,
                resumeIfIdle: true,
            }));
            this._goalContinuationAwaitsRlmWork = false;
    ...
    _runOrQueueGoalContext(kind, images) {
        if (!this._goalState.objective)
            return;
        this._ensureGoalRuntimeActive();
    ...
    async _handleGoalSlashCommand(text, images) {
    ...
    _accountGoalUsageForAssistantMessage(message) {
        if (!this._goalState.objective) {
            return false;
        }
        if (message.stopReason === "error" || message.stopReason === "aborted") {
    ...
    async _shouldStopForThresholdCompaction(context) {
     ...
     */
    async _runSerializedRefineCheckpoint() {
    ...
    async _runSerializedRefineCheckpointAfterBackground(branchVersion) {
        ...
        if (this._compactAutoRefinePending) {
            if (!settings.compact) {
                ...
                const underCooldown = this._lastAutoRefineReviewAt > 0 && nowMs - this._lastAutoRefineReviewAt < settings.cooldownMs;
                if (underCooldown) {
    ...
    async _runSerializedAutoRefineReview(reason, branchVersion) {
     ...
     */
    async _consumeSerializedBackgroundPlan(consume) {
     ...
     */
    async _applySerializedPlan(bgResult) {
        let resolveApplySettled = () => { };
     ...
     */
    async _runBackgroundPlan(options, refineAbort, branchVersion, skipReview = false) {
        try {
     ...
     * is safe.
     */
    async _runSerializedRefine(options, trigger = "manual") {
        if (this._disposed || this._disposing) {
            return;
        }
        ...
        const planSettled = planRun.then(() => undefined, () => undefined);
    ...
    async _thresholdCompactionNeeded(context) {
    ...
    _restoreAutonomousRuntimeSnapshot(snapshot) {
    ...
    async _queueAutonomousContinuationForThresholdCompaction(message) {
        ...
        const text = typeof autonomousMessage.content === "string"
            ? autonomousMessage.content
            : autonomousMessage.content.map((block) => (block.type === "text" ? block.text : "")).join("\n");
        this._admitSessionInput(this._createPreparedTurnAction("followUp", text, undefined, {
    ...
    _queueGoalContinuationForThresholdCompaction(message) {
        if (message.stopReason === "error" || message.stopReason === "aborted") {
            return false;
        }
        if (this._goalState.status !== "active" || !this._goalState.objective) {
        ...
        if (alreadyQueued !== undefined &&
            this._actionStore.unfinishedActions().some((action) => {
                if (action.payload.kind !== "turn" || primaryDeliveryRecord(action).message !== alreadyQueued)
                    return false;
                // A running continuation may already need a successor; only undelivered actions deduplicate.
                return (action.lifecycle.state === "queued" ||
    ...
    _clearQueuedGoalContinuationAfterCancelledThresholdCompaction(queuedGoalContinuation) {
        if (queuedGoalContinuation === undefined)
            return;
        const cancelled = this._cancelSessionActions((action) => action.payload.kind === "turn" && primaryDeliveryRecord(action).message === queuedGoalContinuation, new Error("Queued goal continuation was cleared before delivery."));
    ...
    _clearQueuedAutonomousContinuations(options = {}) {
        ...
        const queuedMessages = this._postCompactionContinuationMessages.filter((message) => requestedMessageSet.has(message));
        if (queuedMessages.length === 0) {
            return;
        }
        const queuedMessageSet = new Set(queuedMessages);
        this._postCompactionContinuationMessages = this._postCompactionContinuationMessages.filter((message) => !queuedMessageSet.has(message));
        this.agent.removeQueuedMessages((message) => queuedMessageSet.has(message));
        this._cancelSessionActions((action) => action.payload.kind === "turn" && queuedMessageSet.has(primaryDeliveryRecord(action).message), new Error("Queued autonomous continuation was cleared before delivery."));
        this._emitQueueUpdate();
        ...
        this._pendingThresholdCompactionAutonomousMessages = this._pendingThresholdCompactionAutonomousMessages.filter((message) => !queuedMessageSet.has(message));
    ...
    _clearQueuedAutonomousContinuationsAfterSkippedThresholdCompaction(shouldContinueAfterThreshold, queuedMessages) {
     ...
     */
    handleGoalHostRequest(type, payload = {}) {
        if (!this._includeGoals) {
     ...
     */
    handleCompactHostRequest(type, payload = {}) {
     ...
     */
    handleRefineHostRequest(type, payload = {}) {
        switch (type) {
            case "refine.status": {
                return {
                    pending: this._pendingRequestedRefine !== undefined,
                    in_flight: this._refineInFlight !== undefined ||
                        this._refinePlanInFlight !== undefined ||
                        this._serializedPlanInFlight !== undefined,
                };
            }
     ...
     */
    handleRlmHeartbeatHostRequest(type, payload = {}) {
        ...
        switch (type) {
            ...
            case "rlm_heartbeat.create": {
                ...
                return {
                    heartbeat: rlmHeartbeatHostResponse(controller.createRlmHeartbeat({
    ...
    handleAgentMessageHostRequest(type, payload = {}) {
    ...
    handleAgentObserveHostRequest(type, payload = {}) {
        ...
        switch (type) {
            ...
            case "agent_observe.recent": {
                ...
                return controller.recentMessages({
                    ...
                    limit: normalizeObserveLimit(payload.limit),
                    maxChars: normalizeObserveMaxChars((payload.max_chars ?? payload.maxChars)),
                });
            }
            default:
    ...
    _createGoalFromHost(objective, tokenBudget) {
        switch (this._goalState.status) {
            case "active":
                throw new Error("cannot create a new goal because this thread already has an active goal; run `await goal.complete()` when it is achieved, or ask the user to clear it with /goal clear");
            case "paused":
                throw new Error("cannot create a new goal because a paused goal exists; ask the user to resume it with /goal resume or clear it with /goal clear");
            case "budget_limited":
                throw new Error("cannot create a new goal because a budget-limited goal exists; ask the user to resume it with /goal resume or clear it with /goal clear");
            default:
    ...
    async _getGoalContinuationMessages(context, signal) {
        if (this._stopGoalContinuationForTerminalMessage(context.message)) {
            return [];
        }
    ...
    async _getContinuationMessages(context, signal) {
    ...
    _agentMessageOutcome(agentMessageId) {
     ...
     */
    waitForAgentMessagePromptDelivery(agentMessageId) {
    ...
    _settleAgentMessage(agentMessageId, leg, error) {
    ...
    _rejectAgentMessage(agentMessageId, error) {
        if (agentMessageId === undefined)
            return;
        this._settleAgentMessage(agentMessageId, "delivery", error);
        this._settleAgentMessage(agentMessageId, "completion", error);
    }
    _rejectQueuedAgentMessageDeliveries(deliveryError, completionError = deliveryError) {
    ...
    _capturingCancelledAction(message) {
        return this._actionStore
            .ownedActions()
            .find((action) => action.lifecycle.state === "cancelled" &&
    ...
    _hasCancelledDispatchCapture() {
        return this._actionStore
            .ownedActions()
            .some((action) => action.lifecycle.state === "cancelled" &&
            action.payload.kind === "turn" &&
    ...
    _handleAgentEvent = (event) => {
        ...
        if (event.type === "message_start" || event.type === "message_end") {
            ...
            for (const action of this._actionStore.ownedActions()) {
                if (action.payload.kind === "turn" && action.payload.captureRunMessages) {
                    for (const message of action.payload.captureRunMessages)
        ...
        if (event.type === "message_start" && (event.message.role === "user" || event.message.role === "custom")) {
            for (const action of this._actionStore.actionsForMessage(event.message)) {
                const record = action.payload.kind === "turn"
    ...
    _addLoginGuidanceToAuthError(event) {
    ...
    async _processAgentEvent(event) {
        ...
        if (event.type === "agent_end") {
            const cleared = this._actionStore
                .ownedActions()
                .filter((action) => action.lifecycle.state === "cancelled" &&
                action.payload.kind === "turn" &&
                action.payload.captureRunMessages !== undefined);
            if (cleared.length > 0) {
                clearedDispatchEnded = true;
                const removed = new Set(cleared.flatMap((action) => action.payload.kind === "turn" ? [...(action.payload.captureRunMessages ?? [])] : []));
     ...
     * before disposal.
     */
    async _drainPendingRefinementForDisposal() {
        ...
        if (this._serializedRefine && this._compactAutoRefinePending && this._autoRefineAllowedForSession()) {
            ...
            if (!compactSettings.enabled || !compactSettings.compact) {
                ...
                const nowMs = Date.now();
                const underCooldown = this._lastAutoRefineReviewAt > 0 && nowMs - this._lastAutoRefineReviewAt < compactSettings.cooldownMs;
                this._compactAutoRefinePending = false;
                if (!underCooldown) {
                    try {
    ...
    async _disposeAsyncOnce() {
        ...
        for (const run of [...this._activeRlmChildRuns.values()]) {
            ...
            if (run.detachedDeletion) {
                ...
                if (!run.settled)
                    await this._finishRlmRunDeletion(run);
            }
            else {
                await childSession.disposeAsync().catch(() => undefined);
            }
    ...
    dispose() {
        ...
        for (const run of this._unsettledRlmChildRuns)
            run.suppressTerminalNotice = true;
        for (const controller of this._rlmQuiescenceWaitAborts)
            controller.abort();
        this._sessionActionCommitDisposeAbortController.abort();
        try {
            ...
            for (const [agentMessageId, outcome] of this._agentMessageOutcomes) {
                if (outcome.delivery)
                    this._settleAgentMessage(agentMessageId, "delivery", deliveryError);
                if (outcome.completion)
                    this._settleAgentMessage(agentMessageId, "completion", completionError);
            }
    ...
    _mergeUnpersistedOutcomes(messages) {
    ...
    async refreshAutonomousGates() {
        await refreshAutonomousQualityGates(this._autonomousState, {
            cwd: this._cwd,
        });
    }
    async _runWithAutonomousContinuationSuppressed(fn) {
    ...
    _markAutonomousContinuationSuppressed(message) {
    ...
    _refreshExtensionSystemPrompt(extensionPrompt, baseSnapshot) {
    ...
    _finishSubmissionNormalization(text, images, policy) {
        let expandedText = text;
        if (policy.expandSkills)
    ...
    _normalizeSubmission(text, images, policy) {
        if (policy.parseSessionCommands) {
            const command = parseSessionSlashCommand(text);
            if (command)
                return { kind: "sessionCommand", text, images, command };
        }
        if (text.startsWith("/")) {
            if (policy.extensionCommands === "execute") {
                const completion = this._executeExtensionCommand(text);
                if (completion)
                    return { kind: "extensionCommand", completion };
            }
    ...
    async _prepareForCommit(policy, steps) {
        ...
        if (policy.preTurnCompaction === "afterModelSelection")
            await this._runPreTurnCompaction();
        const prepared = await steps.prepare();
        if (steps.shouldCommit && !steps.shouldCommit(prepared))
            return undefined;
        steps.beforeFinalRefineBarrier?.(prepared);
        let passedFinalRefineBarrier = false;
        if (policy.finalRefineBarrier === "always" ||
    ...
    _applyPreparedSystemPrompt(preparation, preserveEmptyExtensionPrompt) {
    ...
    async promptUntilAccepted(text, options) {
        return this._prompt(text, { ...options, returnAfterAccepted: true });
    }
    async promptAndWait(text, options) {
        ...
        let cancelQueuedPrompt;
        try {
            await this.promptUntilAccepted(text, { ...options, agentMessageId });
            if (signal) {
                cancelQueuedPrompt = () => {
                    const error = new Error("Prompt was cancelled before it started.");
                    const cancelled = this._cancelSessionActions((action) => action.agentMessageId === agentMessageId && action.payload.kind === "turn", error);
                    if (cancelled.length > 0) {
                        this._settleAgentMessage(agentMessageId, "completion", error);
                    }
                };
    ...
    async queueAgentMessagePrompt(text, streamingBehavior, customMessage) {
        ...
        if (queued && customMessage?.details.fromRelationship === "parent")
            this._repliedToParentSinceTask = false;
        return queued;
    }
    async promptHeartbeat(job, options) {
    ...
    _enqueueRlmTerminalNoticeAction(message) {
        this._assertRlmTerminalNotice(message);
        const action = this._createPreparedTurnAction("followUp", message.content, undefined, {
            message,
        ...
        try {
            ...
            if (!result.accepted)
                throw new Error("RLM child terminal notice was not admitted.");
        }
        catch (error) {
    ...
    async _acquireRlmTerminalNoticeRetentionFence() {
        ...
        while (!this._disposed && !this._disposing && !disposeSignal.aborted) {
            ...
            if (this._queuedWorkPauses.size === 0 && !this._disposed && !this._disposing)
                return fence;
            fence.release();
        }
        return undefined;
    }
    async _deferRlmTerminalNotice(message) {
        this._assertRlmTerminalNotice(message);
    ...
    _demoteRlmTerminalNoticeActions() {
        ...
        this._cancelSessionActions((action) => ids.has(action.id), new Error("RLM child terminal notice deferred across session input suspension."), actions);
        for (const id of ids)
            this._durableRlmTerminalNoticeActionIds.delete(id);
    }
    async _promptInjectedMessage(text, message, options) {
        if (!this.isStreaming && options?.resumeIfIdle)
            this._resumeSessionInputAdmission();
        const admissionEpoch = this._sessionInputPumpEpoch;
        const admissionFence = await this._acquireDirectTurnAdmissionFence(options?.signal).catch((error) => {
        ...
        try {
            ...
            if (visibleQueued && !options?.streamingBehavior) {
                ...
                throw new Error(`${stateDescription}. Specify streamingBehavior ('steer' or 'followUp') to queue the message.`);
            }
            ...
            if (!result.accepted || !result.ticket) {
                if (prefixMessages)
                    this._pendingNextTurnMessages.unshift(...prefixMessages);
            ...
            if (result.disposition === "queued") {
                ...
                void result.ticket.delivered.then(() => reportPreflight(true), () => reportPreflight(false));
            }
            if (options?.returnAfterAccepted) {
                if (result.disposition === "starts_when_admitted")
    ...
    async _prompt(text, options) {
        ...
        const commitFence = this.isStreaming
            ? undefined
            : await this._acquireDirectTurnAdmissionFence(options?.signal).catch((error) => {
                throwIfPromptAdmissionCancelled(options?.signal);
                throw error;
            });
        const reportPreflight = oncePreflight(options?.preflightResult);
        const run = async () => {
            try {
                ...
                const normalizationResult = this._normalizeSubmission(text, options?.images, {
                    ...
                    inputSource: !isInternalPrompt && !options?.skipInputHandlers ? (options?.source ?? "interactive") : undefined,
                ...
                if (normalized.kind === "extensionCommand") {
                    ...
                    void normalized.completion.then(() => this._settleAgentMessage(options?.agentMessageId, "completion"), (error) => this._settleAgentMessage(options?.agentMessageId, "completion", error));
                    void normalized.completion.catch(() => undefined);
                    if (!options?.returnAfterAccepted)
                        await normalized.completion.catch(() => undefined);
                    return;
                }
                ...
                if (normalized.kind === "sessionCommand") {
                    ...
                    const action = this._createSessionCommandAction(normalized.text, normalized.command, normalized.images, schedule, {
                ...
                const deferralObserver = acceptedAgentMessage &&
                    options?.queueIfBusy === true &&
                    !options.streamingBehavior &&
                ...
                if (acceptedAgentMessage && !queueForStreaming && !queueForBusy && !options?.streamingBehavior) {
                    try {
                        const outcome = deferralObserver
                            ? await Promise.race([
                                result.ticket.delivered.then(() => "delivered"),
                                deferralObserver.deferred.then(() => "deferred"),
                            ])
                            : await result.ticket.delivered.then(() => "delivered");
                        if (outcome === "deferred" && !options?.streamingBehavior) {
                            const error = new Error("Agent became busy before prompt delivery. Specify streamingBehavior ('steer' or 'followUp') to queue the message.");
    ...
    async restoreSessionActions(snapshot) {
        ...
        const actions = snapshot.actions.map((recovered) => {
            if (actionIds.has(recovered.id))
                throw new Error(`Duplicate session action id: ${recovered.id}`);
            actionIds.add(recovered.id);
            if (recovered.payload.kind === "turn" &&
            ...
            const payload = recovered.payload.kind === "turn"
                ? {
                    kind: "turn",
    ...
    _restoreSessionCommand(text, customMessage, images, schedule, agentMessageId) {
        if (!isSessionSlashCommandMessage(customMessage) || text !== customMessage.details.command.text) {
            return undefined;
        }
        return this._admitSessionInput(this._createSessionCommandAction(text, customMessage.details.command, images, schedule, {
            agentMessageId,
            source: "internal",
        }), { restore: true }).accepted;
    }
    _restorePromptInput(schedule, snapshot) {
    ...
    async restoreSteeringMessage(text, images, options = {}) {
        if (this._restoreSessionCommand(text, options.customMessage, images, "steer", options.agentMessageId) !== undefined)
            return;
    ...
    async restoreFollowUpMessage(text, images, options = {}) {
        const restoredCommand = this._restoreSessionCommand(text, options.customMessage, images, "followUp", options.agentMessageId);
        if (restoredCommand !== undefined)
    ...
    _buildPromptContent(text, images) {
        const content = [];
    ...
    _deliveryPolicy(schedule) {
        return schedule === "steer" ? "next_turn_boundary" : "when_run_idle";
    }
    _createDeliveryRecord(actionId, role, message) {
        return {
            id: randomUUID(),
            role,
            message,
    ...
    _turnExecutionPolicy(kind, options = {}) {
        if (kind === "queued") {
            return {
                preparation: {
    ...
    _createPreparedTurnAction(schedule, text, images, options) {
        const id = randomUUID();
        ...
        return {
            ...
            wake: options.resumeIfIdle === true
                ? "immediate"
                : schedule === "steer"
                    ? "on_lower_boundary"
                    : "external_resume",
    ...
    _createSessionCommandAction(text, command, images, schedule, options = {}) {
        return {
            id: randomUUID(),
            source: options.source ?? "internal",
            delivery: this._deliveryPolicy(schedule),
    ...
    _coalescedFollowUpOwner(action) {
        if (action.delivery !== "when_run_idle" || action.payload.kind !== "turn" || !action.queueKey)
            return undefined;
        return this._actionStore
            .unfinishedActions()
            .find((candidate) => candidate.queueKey === action.queueKey &&
    ...
    _admitSessionInput(action, options = {}) {
        ...
        if (coalescedOwner) {
            if (action.agentMessageId !== coalescedOwner.agentMessageId) {
                this._rejectAgentMessage(action.agentMessageId, new Error("Prompt was not queued because an equivalent follow-up is already pending."));
            }
        ...
        if (options.front)
            this._actionStore.enqueueFront(action);
        else
            this._actionStore.enqueue(action);
        let disposition = "queued";
        if (canStartImmediately && this._actionStore.selectFirst() === action)
    ...
    _hasSelectableSessionInput() {
        return (this._actionStore.queuedActions().length > 0 ||
    ...
    get hasPendingAdmissionWaiters() {
        return (this._sessionActionCommitOwner !== undefined ||
    ...
    async _pumpSessionInputs(epoch) {
        let blocked = false;
        try {
            while (!this._disposed && !this._disposing && this._hasSelectableSessionInput()) {
                ...
                if (!this._hasCancelledDispatchCapture())
                    await this._agentEventQueue;
                if (!preselected || preselected.payload.kind === "session_command")
                    await this._waitForRefineIdle();
                const activity = this._runtimeActivity();
                const canSelectPreselectedTurn = preselected?.payload.kind === "turn" && canSelectSessionAction({ ...activity, refinementApply: false });
                ...
                try {
                    ...
                    const undelivered = [];
                    for (const action of actions) {
                        if (action.payload.kind !== "turn" || action.lifecycle.state === "cancelled")
                            continue;
                        for (const record of action.payload.records)
                            record.durable ||= delivered.has(record.message);
                        action.payload.records = action.payload.records.filter((record) => {
                            if (record.role === "prefix")
                                return !record.durable;
                            if (record.role === "next_turn")
                                return record.durable;
                            return true;
                        });
                        if (!primaryDeliveryRecord(action).durable)
                    ...
                    for (const action of actions) {
                        const retainedCancelledDispatch = action.lifecycle.state === "cancelled" &&
                            action.payload.kind === "turn" &&
                            action.payload.captureRunMessages !== undefined;
                        if (!retainedCancelledDispatch &&
                ...
                if (epoch !== this._sessionInputPumpEpoch || blocked)
                    return;
            }
        }
        finally {
    ...
    async _executeSelectedSessionCommand(action, epoch) {
        if (action.payload.kind !== "session_command")
            throw new Error("Expected a selected session command");
    ...
    _isDeferredSessionInputError(error, epoch) {
        if (error instanceof DeferredSessionInputError)
            return true;
        if (epoch !== this._sessionInputPumpEpoch)
            return true;
    ...
    async _startPreparedTurnActions(actions, epoch) {
        let nextTurnMessages = [];
        const activeTurns = () => actions.filter((action) => action.payload.kind === "turn" && action.lifecycle.state === "preparing");
        ...
        try {
            const preparedTurn = await this._prepareForCommit(executionPolicy.preparation, {
                ...
                prepare: async () => {
                    ...
                    while (activeTurns().some((action) => action.payload.prepared === undefined)) {
                        ...
                        const result = await this._extensionRunner.emitBeforeAgentStart(preparationAction.payload.text, preparationAction.payload.images, basePromptSnapshot, this._baseSystemPromptOptions);
                        if (activeTurns().at(-1) !== preparationAction)
                            continue;
                        const prepared = { result, basePromptSnapshot };
                        for (const action of activeTurns())
                            action.payload.prepared = prepared;
                    }
                    if (this._isSessionInputHandoffDeferred(epoch)) {
            ...
            try {
                promptPromise = this._sessionActionCommitContext.run(commitFence.owner, () => {
                    ...
                    const contextRecords = nextTurnMessages.map((message) => this._createDeliveryRecord(turns[0].id, "next_turn", message));
                    ...
                    const preparedMessages = turns.flatMap((action) => action.payload.records.map((record) => record.message));
            ...
            await promptPromise;
            if (executionPolicy.completionIncludesRetryChain)
                await this.waitForRetry();
            if (!this._hasCancelledDispatchCapture())
                await this._agentEventQueue;
            if (turns.some((action) => action.lifecycle.state !== "cancelled" &&
    ...
    async _executeQueuedSessionCommand(action) {
        if (action.payload.kind !== "session_command")
            throw new Error("Expected a session command action");
        const input = action.payload;
        try {
            let resultText;
    ...
    _appendDurableSessionCommandMessage(content, command, isResult, isError = false, display = true) {
        ...
        this.sessionManager.appendCustomMessageEntryWithRollback(message.customType, message.content, message.display, message.details);
        this.agent.state.messages.push(message);
        this._emit({ type: "message_start", message });
        this._emit({ type: "message_end", message });
    }
    ...
    clearQueue() {
        ...
        for (const action of clearable) {
            const error = action.payload.kind === "turn" && action.lifecycle.state === "preparing" ? promptError : agentMessageError;
    ...
    clearQueuedUserMessagesMatching(predicate) {
        ...
        const dispatchedTurnCount = ownedActions.filter((action) => action.payload.kind === "turn" &&
            (action.lifecycle.state === "committing" || action.lifecycle.state === "running")).length;
        const matching = ownedActions.filter((action) => action.payload.kind === "turn" &&
        ...
        for (const action of matching) {
            const error = action.payload.kind === "turn" && action.payload.acceptedAgentMessage ? acceptedError : queuedError;
            this._rejectAgentMessage(action.agentMessageId, error);
        }
        for (const [accepted, error] of [
            ...
            if (ids.size > 0)
                this._cancelSessionActions((action) => ids.has(action.id), error, matching);
        }
        if (matching.some((action) => action.lifecycle.state === "cancelled" &&
            action.payload.kind === "turn" &&
            action.payload.captureRunMessages)) {
            this.agent.abort();
        }
    ...
    mutateQueuedMessage(lane, index, expectedText, mutation) {
        ...
        if (!item || queuedAgentMessagePreview(item) !== expectedText)
            return "rejected";
        ...
        if (item.payload.kind === "session_command") {
            ...
            item.payload.text = mutation.text;
            const text = { type: "text", text: mutation.text };
            if (mutation.images !== undefined) {
                item.payload.images = images?.length ? images : undefined;
    ...
    getSessionActionSnapshot() {
        const steering = visibleSessionActionProjection(this._actionStore.queuedActions("next_turn_boundary")).map(queuedAgentMessagePreview);
        const followUps = visibleSessionActionProjection(this._actionStore.queuedActions("when_run_idle")).map(queuedAgentMessagePreview);
        const active = visibleSessionActionProjection(this._actionStore.activeActions())[0];
        const activeState = active?.lifecycle.state;
        const phase = activeState === "selected"
            ? "preparing"
    ...
    getSteeringMessages() {
        return visibleSessionActionProjection(this._actionStore.queuedActions("next_turn_boundary")).map((action) => action.payload.text);
    }
    getSteeringMessagePreviews() {
        return visibleSessionActionProjection(this._actionStore.queuedActions("next_turn_boundary")).map(queuedAgentMessagePreview);
    }
    getFollowUpMessages() {
        return visibleSessionActionProjection(this._actionStore.queuedActions("when_run_idle")).map((action) => action.payload.text);
    }
    getFollowUpMessagePreviews() {
        return visibleSessionActionProjection(this._actionStore.queuedActions("when_run_idle")).map(queuedAgentMessagePreview);
    }
    getSessionActionRecoverySnapshot() {
        return {
            ...
            actions: this._actionStore.snapshotActions().map((action) => ({
                ...
                payload: action.payload.kind === "turn"
                    ? {
                        kind: "turn",
    ...
    async waitForSessionInputCheckpoint(signal) {
        const blocksCheckpoint = () => this._actionStore.activeActions().some((action) => {
            if (action.payload.kind === "session_command") {
                return action.lifecycle.state === "selected" || action.lifecycle.state === "running";
            }
            return (action.lifecycle.state === "selected" ||
                action.lifecycle.state === "preparing" ||
    ...
    async _acquireDirectTurnAdmissionFence(signal) {
    ...
    async _acquireSessionActionCommitFence(signal) {
        const inheritedOwner = this._sessionActionCommitContext.getStore();
        if (inheritedOwner !== undefined && inheritedOwner === this._sessionActionCommitOwner) {
        ...
        let resolve = () => { };
        this._sessionActionCommitTail = new Promise((release) => {
            resolve = release;
        });
        const disposeSignal = this._sessionActionCommitDisposeAbortController.signal;
        const waitSignal = signal ? AbortSignal.any([signal, disposeSignal]) : disposeSignal;
        ...
        return {
            ...
            release: () => {
                if (released)
                    return;
                released = true;
                if (this._sessionActionCommitOwner === owner)
    ...
    async waitForSessionInputIdle() {
        while (true) {
            const pump = this._sessionInputPump;
            await pump;
            if (pump === this._sessionInputPump && !this._sessionInputPumpRequested)
                return;
        }
    }
    async waitForIdle() {
     ...
     */
    async _waitForIdleOrSettlement(settlement) {
        while (settlement === undefined || this._postCompactionContinuationSettlement === settlement) {
            ...
            if (pump === this._sessionInputPump &&
    ...
    getPendingNextTurnMessageSnapshots() {
        ...
        for (const action of this._actionStore.unfinishedActions()) {
            ...
            messages.push(...action.payload.records
                .filter((record) => (record.role === "next_turn" || record.role === "prefix") &&
                record.message.role === "custom" &&
                !record.durable)
                .map((record) => cloneCustomMessage(record.message)));
        }
    ...
    requestAbort() {
        for (const run of [...this._unsettledRlmChildRuns]) {
            if (run.status === "cancelled")
                this._abandonRlmRunForQuiescence(run);
        }
        for (const controller of this._rlmQuiescenceWaitAborts)
            controller.abort();
        this._sessionInputPumpRequested = false;
        ...
        this._cancelSessionActions((action) => action.payload.kind === "turn" &&
            !action.payload.queueVisible &&
            !this._durableRlmTerminalNoticeActionIds.has(action.id), new Error("Prompt aborted before delivery."));
        this._cancelPostCompactionContinue();
        this.abortRetry();
    ...
    _queueModelSelectEmit(nextModel, previousModel, source) {
        const emit = () => this._modelSelectEmitContext.run(true, () => this._emitModelSelect(nextModel, previousModel, source));
    ...
    _trackModelSelectEmitError(emitPromise) {
    ...
    async _cycleScopedModel(direction, options) {
        ...
        const scopedModels = this._scopedModels.filter((scoped) => availableModels.some((model) => modelsAreEqual(model, scoped.model)));
    ...
    _clampServiceTierForModel(serviceTier = this.serviceTier) {
        const effectiveServiceTier = this._getEffectiveServiceTier(serviceTier);
    ...
    async _syncKernelStateAfterCompaction() {
        ...
        if (names === null && !provisioner.hasRunningKernel)
            return;
        const detail = names === null
            ? ""
            : names.length > 0
                ? ` These names are still defined: ${names.join(", ")}.`
                : " You have not defined any names yet.";
        const prunedDetail = pruned && pruned.length > 0
        ...
        const insertBeforeError = last?.role === "assistant" && last.stopReason === "error";
    ...
    _onIpythonStateRestored(result) {
        ...
        if (result.restored.length > 0) {
            lines.push(`Your Python kernel state was revived from your previous session. These names are available again: ${result.restored.join(", ")}.`);
        }
        else {
            lines.push("Your previous Python kernel state could not be revived; the kernel is starting fresh, so re-create any variables, imports, or loaded data you need.");
        }
        if (result.failed.length > 0) {
            lines.push(`These could not be restored and must be recreated if needed: ${result.failed.map((f) => f.name).join(", ")}.`);
        }
    ...
    async compact(customInstructions, options = {}) {
        ...
        let resolveCompactionOperation = () => { };
        const compactionOperation = new Promise((resolve) => {
        ...
        try {
            ...
            if (didCompact) {
                ...
                this._scheduleAutoRefineAfterCompaction(hadPostCompactionContinue || this.agent.hasQueuedMessages() || this.unfinishedActionCount > 0);
            }
        }
    }
    /**
     ...
     */
    async _performCompaction(options) {
        ...
        this.sessionManager.appendCompaction(summary, firstKeptEntryId, tokensBefore, details, fromExtension, customInstructions);
    ...
    async _reapDeletedRlmSubagentRuntimesAfterCompaction() {
        const childIds = [...this._rlmChildCleanupFailures.keys()].filter((childId) => !this._activeRlmChildRuns.get(childId)?.detachedDeletion);
    ...
    _localHarnessStateDir() {
        return (getLocalHarnessStateDir(this.sessionManager.getSessionArtifactDir()) ??
    ...
    _settlePostCompactionContinue(error) {
        if (!error && this._postCompactionContinuationScheduled)
            return;
    ...
    async _invalidatePendingAutoRefineForBranchChange() {
        this._autoRefineReviewAbort?.abort();
        this._discardPendingAutoRefine({ cancelPostCompactionContinue: true });
        this._assistantTurnsSinceAutoRefine = 0;
    ...
    _schedulePostCompactionContinue(continueAfterSessionInput = false) {
    ...
    async _waitForQueuedWorkResume(settlement) {
    ...
    async _runScheduledPostCompactionContinue(settlement) {
        while (this._postCompactionContinuationScheduled && this._postCompactionContinuationSettlement === settlement) {
            ...
            if (waitForSessionInput) {
                ...
                if (this._postCompactionContinuationSettlement !== settlement)
                    return;
                const shouldContinue = (settlement.continueAfterSessionInput && continuationMessages.length === 0) ||
    ...
    _forgetConsumedPostCompactionContinuations(continuationMessages) {
        ...
        this._postCompactionContinuationMessages = this._postCompactionContinuationMessages.filter((message) => !continuationMessageSet.has(message) || stillQueued.has(message));
    }
    ...
    _scheduleAutoRefine(reason, branchVersion = this._autoRefineBranchVersion) {
    ...
    async _maybeAutoRefine(reason) {
        ...
        try {
            ...
            if (!review.shouldRefine) {
                const preserveTurnIntervalReview = reason === "compact" && this._assistantTurnsSinceAutoRefine >= settings.turnInterval;
    ...
    async _runApprovedRefine(reason, review) {
        this._autoRefineInProgress = true;
        try {
    ...
    async _reviewAutoRefine(context, signal) {
        ...
        return reviewAutoRefine(this.agent.state.messages, this._loadMergedHarnessState(), this._loadRefinementHistory(), model, apiKey, context, headers, signal, this.thinkingLevel);
    }
    ...
    _loadMergedHarnessState() {
        ...
        return mergeHarnessStates(loadHarnessState(getGlobalHarnessStateDir(), "global"), localHarnessStateDir ? loadHarnessState(localHarnessStateDir, "local") : undefined);
    }
    _loadRefinementHistory() {
        return mergeRefinementHistory(loadGlobalRefinementHistory(getGlobalHarnessStateDir()), getRefinementHistory(this.sessionManager.getEntries().filter((entry) => entry.type === "custom")));
    }
    /**
     ...
     * applying anything.
     */
    async _planRefine(options, signal, trigger = "manual") {
        if (this._disposed) {
        ...
        const planningState = requestedScope === "global"
            ? globalPlanningState
        ...
        const plan = await planRefinement(this.agent.state.messages, planningState, history, model, apiKey, options, headers, signal, this.thinkingLevel);
        if (this._disposed || signal.aborted) {
            throw new Error("Refinement cancelled because the session was disposed.");
        }
     ...
     */
    async _applyRefine(plan, options, refineAbort) {
        if (this._disposed) {
            throw new Error("Cannot refine a disposed session.");
        }
        ...
        try {
            ...
            let refinementAuditAppendError;
            try {
            ...
            try {
                ...
                if (!refinementAuditAppendError)
                    throw error;
            }
            if (refinementAuditAppendError)
                throw refinementAuditAppendError.error;
            this._baseSystemPrompt = this._rebuildSystemPrompt(this.getActiveToolNames());
            this.agent.state.systemPrompt = this._baseSystemPrompt;
            try {
    ...
    async _checkCompaction(assistantMessage, skipAbortedCheck = true, queueAutonomousContinuation = true) {
        ...
        const compactionEntry = getLatestCompactionEntry(this.sessionManager.getBranch());
        const compactionTimestamp = compactionEntry ? new Date(compactionEntry.timestamp).getTime() : undefined;
        const assistantIsFromBeforeCompaction = compactionTimestamp !== undefined && assistantMessage.timestamp <= compactionTimestamp;
        ...
        if (!assistantIsFromBeforeCompaction &&
            ...
            if (this._overflowRecovery !== "idle") {
                if (this._overflowRecovery === "attempted") {
                    ...
                    this._endCompactionUnsuccessfully("overflow", "failed", "Context overflow recovery failed after one compact-and-retry attempt. Try reducing context or switching to a larger-context model.");
                }
                return false;
            }
        ...
        if (!settings.enabled || assistantIsFromBeforeCompaction)
            return false;
        ...
        if (shouldCompact(contextTokens, contextWindow, settings)) {
            ...
            else if (queueAutonomousContinuation &&
    ...
    _persistCompactionOutcome(reason, outcome, message) {
        ...
        try {
            this.sessionManager.appendCustomMessageEntryWithRollback(outcomeMessage.customType, outcomeMessage.content, outcomeMessage.display, outcomeMessage.details);
        }
        catch (error) {
            const persistenceError = error instanceof Error ? error.message : String(error);
            outcomeMessage = createCompactionOutcomeMessage(`${message}\n\nThis compaction outcome could not be saved to session history: ${persistenceError}`, { reason, outcome });
    ...
    async _runAutoCompaction(reason, willRetry) {
        ...
        const shouldContinueAfterCompaction = (reason === "threshold" || reason === "requested") && this._continueAfterThresholdCompaction;
        const queuedAutonomousContinuationsForThisCompaction = reason === "threshold" && shouldContinueAfterCompaction
            ? this._pendingThresholdCompactionAutonomousMessages.splice(0)
            : [];
        const queuedGoalContinuationForThisCompaction = reason === "threshold" && shouldContinueAfterCompaction ? this._queuedGoalThresholdContinuation : undefined;
        this._continueAfterThresholdCompaction = false;
        ...
        const resumeAfterFailure = () => {
            if ((reason === "requested" || reason === "threshold") &&
        ...
        try {
            ...
            if (!this.model || !authResult || !authResult.ok || !authResult.apiKey) {
                const detail = !this.model || !authResult
            ...
            if (aborted) {
                ...
                this._endCompactionUnsuccessfully(reason, "cancelled", `${reason === "requested" ? "Requested c" : "C"}ompaction cancelled`, { aborted: true, customInstructions });
                return false;
            }
            if (error instanceof CompactionSkippedError) {
                this._endCompactionUnsuccessfully(reason, "skipped", reason === "requested"
                    ? `Requested compaction skipped: ${errorMessage}`
                    : `Auto-compaction skipped: ${errorMessage}`, { errorSeverity: "warning", customInstructions });
                resumeAfterFailure();
                return false;
            }
            this._endCompactionUnsuccessfully(reason, "failed", reason === "overflow"
                ? `Context overflow recovery failed: ${errorMessage}`
                : reason === "requested"
                    ? `Requested compaction failed: ${errorMessage}`
                    : `Auto-compaction failed: ${errorMessage}`, { customInstructions });
            resumeAfterFailure();
            return false;
        }
        finally {
    ...
    _buildRuntime(options) {
        ...
        let configuredBaseToolDefinitions;
        if (this._baseToolsOverride) {
            configuredBaseToolDefinitions = Object.fromEntries(Object.entries(this._baseToolsOverride).map(([name, tool]) => [
                name,
        ...
        this._baseToolDefinitions = new Map(Object.entries(configuredBaseToolDefinitions).map(([name, tool]) => [name, tool]));
        ...
        const hasSnapshot = !!this._ipythonKernelSnapshotDir && existsSync(snapshotPathIn(this._ipythonKernelSnapshotDir));
    ...
    _createKernelHostHandlers() {
        ...
        const visibleKernelSkillNames = new Set(this._modelVisibleSkills()
            .filter((skill) => !skill.disableModelInvocation)
            .map((skill) => skill.name));
        if (this._agentMessageController && visibleKernelSkillNames.has(AGENT_MESSAGE_SKILL_NAME)) {
            Object.assign(handlers, createAgentMessageHostHandlers({
                roster: async () => (await this.handleAgentMessageHostRequest("agent_message.list_agents")),
                    ...
                    if (this._rlmDepth > 0) {
                        ...
                        if (input.receiverRole === undefined && this._agentMessageController?.roster) {
                            try {
                                ...
                                addressedParent = roster.entries.some((entry) => entry.relationship === "parent" &&
        ...
        if (this._agentObserveController) {
            Object.assign(handlers, createAgentObserveHostHandlers({
                listAgents: () => this.handleAgentObserveHostRequest("agent_observe.list"),
                getAgent: (target) => this.handleAgentObserveHostRequest("agent_observe.get", {
                    target,
                }),
                recentMessages: (input) => this.handleAgentObserveHostRequest("agent_observe.recent", {
                    target: input.target,
                    limit: input.limit,
    ...
    _createRlmSubagentRuntimeOptions(options) {
        return {
            ...
            serviceTier: this.serviceTier === "priority" && !supportsFastMode(options.model) ? "default" : this.serviceTier,
    ...
    async _createRlmSubagentRuntime(options) {
    ...
    async _awaitPendingRlmChildPublication(selector) {
        const run = [...this._activeRlmChildRuns.values()].find((candidate) => (candidate.status === "queued" || candidate.status === "running" || candidate.status === "done") &&
    ...
    _buildRlmSubagentList(listedAgents) {
        ...
        for (const [childId, { session: childSession }] of this._rlmChildSessions) {
            if (this._deletingRlmChildren.has(childId) ||
            ...
            subagents.push({
                ...
                session_name: daemonChild?.sessionName ?? childSession.sessionName ?? createDefaultRlmSubagentSessionName("", childId),
    ...
    async deleteInactiveRlmSubagent(childId, isExternallyRunning = () => false) {
        const isRunning = () => {
        ...
        const subagent = [...(await this.listRlmSubagents()).subagents, ...this._rlmChildCleanupFailures.values()].find((entry) => entry.rlm_child_id === childId);
        if (!subagent) {
            for (const run of this._activeRlmChildRuns.values()) {
    ...
    async deleteRlmSubagent(target) {
        const inFlight = [...this._deletingRlmChildren.values()].filter(({ subagent }) => this._rlmSubagentMatchesTarget(subagent, target));
        ...
        if (localMatches[0]) {
            ...
            return this._trackRlmSubagentDeletion(subagent, async () => {
                ...
                const passiveMatches = listedSubagents.filter((entry) => entry.rlm_child_id !== subagent.rlm_child_id && this._rlmSubagentMatchesTarget(entry, target));
                if (passiveMatches.length > 0) {
                    throw new Error(`RLM subagent selector "${target}" is ambiguous in the current parent session`);
                }
                const parentActiveSessionId = listedAgents?.current?.activeSessionId;
                const daemonChild = listedAgents?.agents.find((agent) => agent.rlmChildId === subagent.rlm_child_id && agent.parentActiveSessionId === parentActiveSessionId);
    ...
    async _trackRlmSubagentDeletion(subagent, startDeletion) {
    ...
    _ensureRlmRunDeletionCleanup(run, session) {
        if (run.deletionCleanup)
            return run.deletionCleanup;
    ...
    async _recordRlmRunDeletionCleanupFailure(run, subagent, session, error) {
        if (this._disposed || this._disposing) {
            run.suppressTerminalNotice = true;
            await session.disposeAsync().catch(() => undefined);
            if (!run.settled)
    ...
    async _finishRlmRunDeletion(run) {
    ...
    _observeRlmRunDeletionCleanup(run, subagent, session, cleanup) {
        if (run.deletionCleanupObserver)
            return run.deletionCleanupObserver;
        const observer = cleanup.then(() => true, async (error) => {
    ...
    async _deleteResolvedRlmSubagent(subagent) {
        ...
        if (run) {
            ...
            if (liveSession)
                this._continueFinishedRlmRunDeletion(run, subagent, liveSession);
     ...
     */
    registerRlmChildSession(childId, session, unsubscribe) {
    ...
    releaseRlmChildSession(childId, session) {
        ...
        if (this._rlmChildSessions.get(childId)?.session !== session)
            return false;
    ...
    _rlmChildSnapshotForRun(run, child = run.session ?? this._rlmChildSessions.get(run.id)?.session) {
    ...
    _rlmChildSnapshotForSession(childId, child) {
        let answerPreview;
        let toolUseCount = 0;
        const messages = child.state.streamingMessage?.role === "assistant"
    ...
    getRlmChildSnapshots() {
        ...
        for (const run of this._activeRlmChildRuns.values()) {
            const hidden = run.detachedDeletion || this._deletingRlmChildren.has(run.id) || this._deletedRlmChildIds.has(run.id);
        ...
        for (const [childId, { session: child, run }] of this._rlmChildSessions) {
            if (recorded.has(childId) || traversed.has(childId))
                continue;
    ...
    _rlmChildSessionSnapshot() {
        ...
        for (const [childId, { session }] of this._rlmChildSessions) {
            if (!this._abandonedRlmQuiescenceChildIds.has(childId))
                sessions.add(session);
        }
        for (const run of this._activeRlmChildRuns.values()) {
            if (run.session && !run.abandonedForQuiescence)
                sessions.add(run.session);
        }
        return [...sessions];
    }
    _hasUnsettledRlmQuiescenceWork() {
        if (this._hasDeferredRlmTerminalNotices())
            return true;
        if ([...this._unsettledRlmChildRuns].some((run) => !run.settled))
            return true;
        return this._rlmChildSessionSnapshot().some((child) => child.isSessionActive || child._hasUnsettledRlmQuiescenceWork());
    }
    /**
     ...
     */
    async waitForRlmQuiescence(externalSignal) {
        const cancellation = new AbortController();
        const cancelFromParent = () => cancellation.abort();
        if (externalSignal?.aborted)
            cancellation.abort();
        else
            externalSignal?.addEventListener("abort", cancelFromParent, { once: true });
        this._rlmQuiescenceWaitAborts.add(cancellation);
        let rejectCancelled = (_error) => { };
        ...
        try {
            while (true) {
                ...
                if (unsettledRuns.length === 0 && !this._hasUnsettledRlmQuiescenceWork())
                    return;
     ...
     */
    cancelRlmChildRun(childId, reason = "Cancelled by user") {
        const run = this._activeRlmChildRuns.get(childId);
        if (run) {
    ...
    async _assertRlmSubagentSessionNameAvailable(name, ignorePendingReservation = false) {
        ...
        const localConflict = [...this._activeRlmChildRuns.values()].some((run) => run.session?.sessionName === name || (!run.session && run.sessionName === name)) ||
        ...
        const catalog = listed.agents.map((agent) => ({
            id: agent.sessionId,
    ...
    async findRlmModels(query, limit) {
        return {
    ...
    async _resolveRlmSubagentModel(reference) {
        ...
        const model = (await this._authenticatedRlmModels()).find((candidate) => `${candidate.provider}/${candidate.id}`.toLowerCase() === normalizedReference);
        if (!model) {
    ...
    async _startRlmChildRun(prompt, kwargs = {}, spawnCode) {
        ...
        if (requestedThinkingLevel !== undefined) {
            ...
            if (!supported.includes(requestedThinkingLevel)) {
                throw new Error(`Requested thinking level "${requestedThinkingLevel}" is not supported by model "${modelSelection.model.provider}/${modelSelection.model.id}"; supported levels: ${supported.join(", ")}`);
            }
        }
        if (this._disposed || this._disposing)
        ...
        emitChildUpdate();
        const publishChildSession = (child) => {
            childSession = child;
            if (this._activeRlmChildRuns.get(run.id) !== run)
                return;
            run.session = child;
        ...
        const deliverTerminalMessageToParent = async (message) => {
        ...
        run.completeDeletion = () => {
            ...
            if (run.deletionNotice)
                return run.deletionNotice;
            const notice = deliverTerminalMessageToParent(createRlmChildTerminalNoticeMessage({
                kind: "cancelled",
                childId: run.id,
                sessionName,
        ...
        run.reportDeletionCleanupFailure = (error) => {
            ...
            return deliverTerminalMessageToParent(createRlmChildFailureMessage({
                childId: run.id,
                sessionName,
        ...
        void (async () => {
            ...
            try {
                ...
                if (run.status === "cancelled")
                    throw new Error(run.error ?? "RLM child cancelled");
                if (child.sessionName !== sessionName)
                    child.setSessionName(sessionName);
                        ...
                        if (assistant.stopReason !== "error" && assistant.stopReason !== "aborted") {
                            ...
                            if (parentAssistantForUsage) {
                                ...
                                if (parentEntry) {
                                    ...
                                    const origin = precedingPrompt?.role === "custom" && isAgentSessionMessage(precedingPrompt)
                                        ? precedingPrompt.details.id.startsWith("spawn:")
                                            ? "spawn_task"
                                            : "agent_message"
                                        : "direct_user";
                                    this.sessionManager.appendChildUsageAttribution(parentEntry.id, assistant.usage, parentAssistantForUsage.usage, origin);
                                }
                            }
                        }
                        const text = compactRlmText(readAssistantText(assistant));
                        if (text)
                            run.answerPreview = text;
                ...
                if (!run.detachedDeletion && !run.suppressTerminalNotice) {
                    if (run.status === "error") {
                        await deliverTerminalMessageToParent(createRlmChildFailureMessage({
                            childId: run.id,
                            sessionName,
                ...
                if (!run.detachedDeletion && childSession && this._subagentRuntimeHost?.releaseRlmSubagentRuntime) {
                    try {
                        await this._subagentRuntimeHost.releaseRlmSubagentRuntime(childRuntime ?? { session: childSession }, subagentOptions, run.status === "cancelled" ? "cancelled" : "error");
                ...
                if (run.detachedDeletion) {
                    ...
                    if (!run.settled) {
                        ...
                        if (childRuntime && cleanupSucceeded) {
                            const cleanup = run.deletionCleanup ?? this._ensureRlmRunDeletionCleanup(run, childRuntime.session);
                            cleanupSucceeded = await this._observeRlmRunDeletionCleanup(run, run.detachedDeletion, childRuntime.session, cleanup);
                        }
                        if (cleanupSucceeded)
                            await this._finishRlmRunDeletion(run);
                    }
    ...
    async runRlmChild(prompt, kwargs = {}, spawnCode) {
    ...
    _getProviderStreamFailureDetails(message) {
    ...
    _isStructuredPermanentProviderFailure(message) {
    ...
    _getProviderStreamFailureAuthStatus(message) {
    ...
    _isConcreteProviderAuthFailure(message) {
        ...
        return (/\b(?:401|403)\b/.test(message.errorMessage) &&
    ...
    _captureRetryAuthFailureSource(message) {
        ...
        if (!this._retryAuthFailureSources.some((existing) => existing.provider === token.provider &&
    ...
    _markProviderAuthStaleForRetryFailure(message, options) {
        const authSourceTokens = this._retryAuthFailureSources.length > 0 ? this._retryAuthFailureSources : options?.authSourceTokens;
     ...
     * @param options.excludeFromContext If true, command output won't be sent to LLM (!! prefix)
     */
    async runUserBash(command, options) {
        ...
        let end;
        try {
            end = await this.runUserBashLocked(command, options?.excludeFromContext ?? false, options?.transient ?? false, identity);
        }
        finally {
    ...
    async _drainQueuedMessagesAfterBash() {
        await this.agent.waitForIdle();
        this._scheduleSessionInputPump();
    }
    async runUserBashLocked(command, excludeFromContext, transient, identity) {
     ...
     */
    _branchNavigationQueue = Promise.resolve();
    async navigateTree(targetId, options = {}) {
        const previous = this._branchNavigationQueue;
        let release = () => { };
        this._branchNavigationQueue = new Promise((resolve) => {
            release = resolve;
        });
        await previous;
        try {
    ...
    async _navigateTreeUnderPause(targetId, targetEntry, options) {
        ...
        const branchSummaryOperation = new Promise((resolve) => {
    ...
    getContextTree() {
        ...
        const { ownUsage, totalUsage } = computeOwnAndTotalUsage(this.sessionManager.getBranch(), this.sessionManager.getEntries());
        const children = [];
        const liveIds = new Set();
        for (const run of this._activeRlmChildRuns.values()) {
            liveIds.add(run.id);
            const node = run.session?.getContextTree() ?? loadContextTreeChildFromDisk(run.sessionDir, resolveContextWindow);
            children.push({
                ...(node ?? {
                    ownUsage: emptyUsage(),
...
function isRlmHeartbeatStatusUpdate(value) {
    return value === "pause" || value === "resume";
}
function rlmHeartbeatHostResponse(job) {
    return {
        id: job.id,
        status: job.status,
