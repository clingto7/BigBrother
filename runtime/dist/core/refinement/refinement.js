...
const TRUNCATED_JSON_ERROR = "the model stopped before completing its JSON object. This usually means the output budget was exhausted; retry with a smaller request.";
function refinementMaxOutputTokens(model) {
    return Math.min(model.maxTokens, REFINEMENT_MAX_OUTPUT_TOKENS);
}
function autoRefineReviewMaxOutputTokens(model) {
...
function normalizeHarnessScope(value, fallback) {
    return value === "global" || value === "local" ? value : fallback;
}
export function inferRefinementResultScope(result) {
    if (result.scope) {
        return result.scope;
    }
...
function withDefaultRefinementScope(result, scope) {
    const inferred = inferRefinementResultScope(result);
    return { ...result, scope: inferred ?? scope };
}
export function getGlobalHarnessStateDir(agentDir = getAgentDir()) {
    return join(agentDir, HARNESS_STATE_DIR_NAME);
}
export function getLocalHarnessStateDir(sessionArtifactDir) {
    return sessionArtifactDir ? join(sessionArtifactDir, HARNESS_STATE_DIR_NAME) : undefined;
}
export function getHarnessStatePath(harnessStateDir = getGlobalHarnessStateDir()) {
    return join(harnessStateDir, "harness_state.json");
}
export function loadHarnessState(harnessStateDir = getGlobalHarnessStateDir(), scope = "global") {
...
export function saveHarnessState(harnessStateDir, state) {
    const statePath = getHarnessStatePath(harnessStateDir);
...
export function getRefinementHistoryPath(harnessStateDir = getGlobalHarnessStateDir()) {
    return join(harnessStateDir, REFINEMENT_HISTORY_FILE_NAME);
}
function isRefinementResult(data) {
 ...
 */
export function appendGlobalRefinement(harnessStateDir, result) {
    const historyPath = getRefinementHistoryPath(harnessStateDir);
    mkdirSync(harnessStateDir, { recursive: true });
    appendFileSync(historyPath, `${JSON.stringify(result)}\n`, "utf8");
    return historyPath;
}
export function loadGlobalRefinementHistory(harnessStateDir = getGlobalHarnessStateDir()) {
    const historyPath = getRefinementHistoryPath(harnessStateDir);
 ...
 */
export function mergeRefinementHistory(global, session) {
    const byId = new Map();
...
export function formatHarnessStateForPrompt(state, options = {}) {
    ...
    for (const kind of Object.keys(state.entries)) {
        ...
        if (kind === "subagent" && entries.length > 0 && includeIpythonExamples) {
            lines.push(`${kind}: ${entries.length} (invoke a spec by turning it into a concise task prompt and spawning with \`await rlm('<task>')\`; admission returns a child handle, never the answer)`);
        }
        else {
        ...
        for (const entry of entries.slice(0, maxEntriesPerKind)) {
            ...
            const referenceText = entry.kind === "skill" && Object.keys(entry.reference).length > 0
                ...
                : "";
            lines.push(`- [${entry.scope ?? "global"}:${entry.id}] ${entry.title} (${entry.path}, v${entry.version})${referenceText}${argumentsText}: ${compactText(entry.content, maxContentLength)}`);
        }
    ...
    for (const kind of Object.keys(state.entries)) {
        ...
        for (const entry of entries.slice(0, 40)) {
            ...
            const referenceText = entry.kind === "skill" && Object.keys(entry.reference).length > 0
                ...
                : "";
            lines.push(`- [${entry.scope ?? "global"}:${entry.id}] ${entry.title} (${entry.path}, v${entry.version})${referenceText}${argumentsText}: ${content}`);
        }
 ...
 */
function isIncompleteJson(candidate) {
    let depth = 0;
    let inString = false;
    let escaped = false;
 ...
 */
export function normalizeRefinementProposal(value) {
    ...
    return {
        ...
        edits: edits
            .filter((edit) => typeof edit === "object" && edit !== null)
            .map((edit) => ({
            action: edit.action,
            kind: edit.kind,
            ...
            metadata: typeof edit.metadata === "object" && edit.metadata !== null && !Array.isArray(edit.metadata)
                ? edit.metadata
                : undefined,
...
function validateEdit(edit, computedId) {
    ...
    if (edit.action !== "delete" && edit.kind === "skill") {
        ...
        const hasImport = (typeof reference.import === "string" && reference.import.length > 0) ||
            (typeof reference.python_import === "string" && reference.python_import.length > 0);
        const hasCallable = (typeof reference.callable === "string" && reference.callable.length > 0) ||
...
export function applyRefinementProposal(state, proposal, options) {
    const appliedEdits = [];
    const proposalModifiedKeys = new Set();
...
function rollbackProposal(target) {
    const edits = [];
...
export function getRefinementHistory(entries) {
    return entries
...
export function generateRefinementId() {
    return `refine_${new Date()
        .toISOString()
        .replace(/[^0-9]/g, "")
        .slice(0, 17)}`;
}
export async function planRefinement(messages, state, history, model, apiKey, options = {}, headers, signal, thinkingLevel) {
    ...
    if (options.rollbackId) {
        ...
        const fallbackScope = options.global ? "global" : "local";
        return {
...
function parseAutoRefineReview(text) {
    const value = extractJsonObject(text);
    if (typeof value !== "object" || value === null || Array.isArray(value)) {
...
export async function reviewAutoRefine(messages, state, history, model, apiKey, context, headers, signal, thinkingLevel) {
...
export async function refineHarness(messages, state, history, model, apiKey, options = {}, headers, signal, thinkingLevel) {
