...
import { KernelBusyAfterInterruptError, ReplKernelManager, } from "../kernel/index.js";
import { manifestPathIn, snapshotPathIn } from "../kernel/state-snapshot.js";
...
try:
    ...
    def bash(command):
        rlm._raise_missing()
`.trim();
export function buildRlmBootstrapCode(pythonSkills = []) {
...
function createLinkedAbortSignal(sources) {
...
function setWorkingMessage(ctx, message) {
    try {
    ...
    async startKernel(signal) {
        ...
        try {
            if (this.options?.readyGate) {
                await raceWithAbort(this.options.readyGate.catch(() => { }), startupSignal);
            }
...
async function chooseBusyKernelAction(ctx, signal) {
    if (!ctx?.hasUI) {
        return "cancel";
    }
...
async function executeWithBusyKernelChoice(provisioner, reportStartupProgress, toolCallId, code, signal, onStream, onWorkingMessage, onLateSentAgentMessage, ctx) {
...
export function createIpythonToolDefinition(cwd, options) {
    ...
    return {
        ...
        execute: async (toolCallId, params, signal, onUpdate, ctx) => {
            ...
            const setToolWorkingMessage = (message) => {
                setWorkingMessage(ctx, message);
                hasWorkingMessage = message !== undefined;
            };
            const reportStartupProgress = (message) => {
            ...
            try {
                const { result: r, kernelRestarted } = await executeWithBusyKernelChoice(provisioner, reportStartupProgress, toolCallId, params.code, signal, (chunk) => {
                    onUpdate?.({
                        content: [{ type: "text", text: chunk }],
                        details: { status: "ok" },
                    });
                }, setToolWorkingMessage, options?.onLateSentAgentMessage, ctx);
                let text = r.stdout;
                if (r.stderr)
                    text += (text ? "\n" : "") + r.stderr;
                if (r.result)
                ...
                const content = [{ type: "text", text: text || "" }, ...imageBlocks];
                return {
                    content,
                    details: {
