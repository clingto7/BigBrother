...
const PYTHON_ASSIGNMENT_CALL_PATTERN = /^\s*[A-Za-z_][A-Za-z0-9_]*(?:\s*:\s*[^=]+)?\s*=\s*(?:await\s+)?[A-Za-z_][A-Za-z0-9_.]*\s*\(/;
const PYTHON_LOW_SIGNAL_ASSIGNMENT_CALL_PATTERN = /^\s*[A-Za-z_][A-Za-z0-9_]*(?:\s*:\s*[^=]+)?\s*=\s*(?:await\s+)?(?:Path|pathlib\.Path|json\.loads|json\.dumps|str|int|float|list|dict|set|tuple)\s*\(/;
const PYTHON_EFFECT_CALL_PATTERN = /^\s*(?:await\s+)?[A-Za-z_][A-Za-z0-9_.]*\.(?:write_text|write_bytes|mkdir|unlink|rename|replace|touch|append|extend|update|add|remove|discard|close|commit|execute|run)\s*\(/;
...
function simplifyRunnerCommand(line) {
    ...
    const pytestIndex = words.findIndex((word, index) => word === "pytest" || (word === "pytest" && words[index - 1] === "-m"));
...
function simplifyMutationCommand(line) {
    const words = shellWords(line);
    if (words.length === 0)
        return undefined;
    if (words[0] === "cat" && words[1] === ">" && words[2])
        return `write ${pathTail(words[2])}`;
    if (words[0] === "tee" && words.at(-1))
        return `${words.includes("-a") ? "append" : "write"} ${pathTail(words.at(-1) ?? "")}`;
    if (words[0] === "apply_patch")
        return "apply patch";
    if (["rm", "mv", "cp", "git", "npm"].includes(words[0] ?? ""))
        return line;
    if ((words[0] === "sed" && words.some((word) => word.startsWith("-i"))) ||
        (words[0] === "perl" && words.includes("-pi"))) {
        return line;
    }
    return undefined;
}
function simplifyBashCommandLine(line) {
...
function heredocBody(lines, startIndex, delimiter) {
...
function isSkippablePythonLine(line) {
    const trimmed = line.trim();
...
function pythonPrintInnerCall(line) {
...
function pythonFileOperation(line, paths) {
    const match = line
        .trim()
        .match(/^(?:await\s+)?([A-Za-z_][A-Za-z0-9_]*)\.(write_text|write_bytes|read_text|read_bytes|mkdir|unlink|rename|replace|touch)\s*\(/);
    if (!match?.[1] || !match[2])
        return undefined;
...
function pythonSubprocessCommand(line) {
    const trimmed = line.trim();
    const shellMatch = trimmed.match(/subprocess\.(?:run|check_call|check_output|Popen)\(\s*["`]([^"`]+)["`]/);
    if (shellMatch?.[1])
        return simplifyBashCommandLine(shellMatch[1]);
...
function simplifyPythonPreviewLine(line, paths) {
    return (pythonFileOperation(line, paths) ?? pythonSubprocessCommand(line) ?? pythonPrintInnerCall(line) ?? line.trim());
}
function pythonPreviewLine(lines, index, paths) {
...
function firstPythonChildLine(lines, parentIndex) {
...
function pythonLineScore(lines, index, paths) {
    const line = lines[index] ?? "";
    const trimmed = line.trim();
...
function pythonPreviewIndex(lines, index) {
    const line = lines[index] ?? "";
    if (!PYTHON_CONTROL_PATTERN.test(line)) {
        return index;
    }
...
export function previewPythonCode(code) {
...
export function previewIpythonCode(code) {
