...
function pythonImportNameForSkill(name) {
    return name.replaceAll("-", "_");
}
function isValidPythonImportName(name) {
    return /^[A-Za-z_][A-Za-z0-9_]*$/.test(name);
}
function detectPythonSkill(skillDir, name, diagnostics) {
...
export function getPythonSkillRuntimeInfo(skills) {
    return skills
        .filter((skill) => skill.kind === "python")
        .map((skill) => ({
        name: skill.name,
...
export function loadSkills(options) {
    ...
    const pythonImportMap = new Map();
    const allDiagnostics = [];
    const collisionDiagnostics = [];
    const pythonImportDiagnostics = [];
