...
export const DEFAULT_AUTONOMOUS_GATES = {
    commands: [],
    maxRetries: 3,
    timeoutMs: 5 * 60 * 1000,
};
...
export function createAutonomousRuntimeState(config, _options = {}) {
...
export function setAutonomousEnabled(state, enabled, _options = {}) {
...
export function autonomousStatus(state) {
    return {
...
export function addAutonomousUsage(state, usage) {
    if (!state.enabled) {
        return;
    }
    state.turnsUsed++;
...
export function addAutonomousContinuation(state) {
    if (!state.enabled) {
        return;
    }
    state.continuationsUsed++;
}
function autonomousTokenDelta(usage) {
    if (!usage) {
        return 0;
    }
...
export async function nextAutonomousContinuation(state, message, options = {}, now = Date.now()) {
    ...
    return {
        ...
        content: [
            {
                type: "text",
                text: decision.reason === "gate_failed"
...
export async function shouldAutonomouslyContinue(state, message, options = {}, now = Date.now()) {
    options.signal?.throwIfAborted();
...
export function autonomousLimitReason(state, now = Date.now()) {
...
export async function refreshAutonomousQualityGates(state, options = {}) {
    options.signal?.throwIfAborted();
...
async function runAutonomousQualityGates(state, cwd, signal) {
...
export function buildAutonomousGateFailureContinuation(failure, maxRetries, timestamp = Date.now()) {
    return (`Autonomous quality gate failed (attempt ${failure.attempt}/${maxRetries}): \`${failure.command}\` ${failure.exitText}.\n` +
        (failure.output ? `\nOutput:\n${failure.output}\n` : "\n") +
        `\nContinue working. Fix the failure, then produce terminal evidence. Timestamp: ${new Date(timestamp).toISOString()}.`);
}
function buildGateFailureContinuation(state, timestamp) {
...
function gitWorktreeSnapshotsEqual(a, b) {
    return !!a && !!b && a.status === b.status && a.diff === b.diff && a.untrackedHash === b.untrackedHash;
}
async function captureGitWorktreeSnapshot(cwd, signal) {
    signal?.throwIfAborted();
    if (!cwd) {
        return undefined;
    }
    ...
    const status = await runChildProcess("git", ["--no-optional-locks", "status", "--porcelain=v1", "-z", "-uall", "--no-renames", ...pathspec], {
        cwd,
        timeoutMs: 10_000,
        signal,
    });
    ...
    const diff = await runChildProcess("git", ["--no-optional-locks", "diff", "--no-ext-diff", "--binary", "HEAD", ...pathspec], {
        cwd,
        timeoutMs: 10_000,
        signal,
    });
...
function untrackedPathsFromStatus(status) {
    return status
        .split("\0")
...
async function hashUntrackedFiles(cwd, status, signal) {
...
async function hashUntrackedPath(path, signal) {
    try {
        signal?.throwIfAborted();
...
function runChildProcess(command, args, options = {}) {
    ...
    return new Promise((resolve) => {
        ...
        void waitForChildProcess(child).then((status) => finish({ status, signal: child.signalCode }), (err) => {
            error = err;
...
function truncateGateOutput(output, outputAlreadyTruncated = false, maxChars = MAX_GATE_OUTPUT_CHARS) {
