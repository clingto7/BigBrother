...
export async function runRpcModeWithConnection(connection) {
    return runRpcModeWithConnectionInternal(connection);
}
async function runRpcModeWithConnectionInternal(connection, options = {}) {
    takeOverStdout();
    const output = (value) => {
        writeRawStdout(serializeJsonLine(value));
    };
    const success = (id, command, data) => (data === undefined
    ...
    const bufferedConnectionOutputs = [];
    const pendingConnectionUiRequests = new Set();
    const signalCleanupHandlers = [];
    const extensionUi = createRpcExtensionUiBridge(output);
    const observationCommandTails = new Map();
    const observations = new Map();
    const isDialogMethod = (method) => method === "select" || method === "confirm" || method === "input" || method === "editor";
    const outputConnectionEvent = (event) => {
    ...
    const stopObservation = async (activeSessionId) => {
    ...
    const activateObservation = (activeSessionId) => {
        const observation = observations.get(activeSessionId);
    ...
    for (const signal of ["SIGTERM", ...(process.platform === "win32" ? [] : ["SIGHUP"])]) {
        const handler = () => {
    ...
    const handleCommand = async (command) => {
        ...
        switch (command.type) {
            ...
            case "abort":
                ...
                return success(id, command.type);
            case "new_session":
                return success(id, command.type, await connection.newSession(command.parentSession ? { parentSession: command.parentSession } : undefined));
            case "get_state": {
            ...
            case "refine":
                return success(id, command.type, await connection.refine({
            ...
            case "send_message":
                return success(id, command.type, await connection.sendAgentMessage(command.targetActiveSessionId, command.message));
            ...
            case "unobserve":
                ...
                return success(id, command.type);
            case "get_commands": {
                const commands = (await connection.getCommands()).map((available) => ({
                    name: available.name,
    ...
    const handleInputLine = async (line) => {
        ...
        if (parsed.type === "extension_ui_response") {
            ...
            if (extensionUi.handleResponse(response))
                return;
            pendingConnectionUiRequests.delete(response.id);
            const daemonResponse = "cancelled" in response && response.cancelled
                ? { cancelled: true }
                : "value" in response
        ...
        if (command.type === "observe" || command.type === "unobserve") {
            ...
            const tail = ordered.then(() => undefined, () => undefined);
        ...
        if (command.type === "prompt") {
            ...
            promptCommandTail = ordered.then(() => undefined, () => undefined);
            await ordered;
            return;
        }
        await executeCommand();
    };
    const pendingInputHandlers = new Set();
    const dispatchInputLine = (line) => {
