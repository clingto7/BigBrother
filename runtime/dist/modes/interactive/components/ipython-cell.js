import { truncateToWidth, VersionedRenderCache, visibleWidth, wrapTextWithAnsi, } from "@earendil-works/pi-tui";
...
export function getIpythonCodeFromArgs(args) {
...
function readSentAgentMessages(value) {
...
function readDiffDisplays(value) {
    if (!Array.isArray(value)) {
        return undefined;
    }
    const diffs = value.flatMap((entry) => {
        if (!entry || typeof entry !== "object") {
            return [];
        }
        const record = entry;
...
function isEditConfirmation(text, diffs) {
    if (!text) {
        return false;
    }
 ...
 */
function isAgentMessageReceipt(text, messages) {
    if (!text || messages.length === 0) {
        return false;
    }
    const stripped = stripReprQuotes(text);
    return messages.some((message) => stripped.startsWith(`{'id': '${message.id}'`) || stripped.startsWith(`{"id": "${message.id}"`));
}
function readErrorDetails(value) {
    ...
    return {
        ...
        traceback: Array.isArray(record.traceback)
            ? record.traceback.filter((line) => typeof line === "string")
            : [],
    };
}
...
function splitTraceback(text, errorName) {
...
function formatIpythonErrorSummary(error) {
...
export class IPythonCellComponent {
    renderCache = new VersionedRenderCache();
    state;
    stateVersion = 0;
    constructor(state) {
        this.state = state;
    ...
    render(width) {
        ...
        const cacheVersion = this.statusKind(details) === "running"
    ...
    hasResult(details) {
        return (details.stdout !== undefined ||
    ...
    renderCode(lines, width) {
        ...
        for (const [index, rawLine] of rawLines.entries()) {
            ...
            const highlighted = isBashCell || MAGIC_LINE_PATTERN.test(rawLine) || parseIpythonBashCell(rawLine) !== undefined
    ...
    highlightInputLine(line, isBashCell) {
    ...
    renderOutput(lines, width, details, hasCode) {
        const blocks = this.state.content ?? [];
        const text = textFromBlocks(blocks);
        const imageCount = blocks.filter(isImageBlock).length;
        const hasStructuredOutput = details.stdout !== undefined ||
            details.stderr !== undefined ||
            details.result !== undefined ||
            details.error !== undefined;
        const traceback = !hasStructuredOutput && (this.state.isError || details.status === "error")
        ...
        const backgroundOutput = hasStructuredOutput && details.backgroundOutput?.trim() ? details.backgroundOutput : undefined;
        if (backgroundOutput) {
        ...
        if (details.error) {
            startOutput();
            this.renderTraceback(lines, width, details.error.traceback.join("\n") || formatIpythonErrorSummary(details.error));
        }
        else if (traceback) {
            startOutput();
    ...
    renderSentAgentMessages(lines, width, messages) {
        for (const message of messages) {
            ...
            this.addPlain(lines, truncateToWidth(agentMessageSummaryLine(label, recipient, preview), Math.max(1, width - 1), "…"));
        }
    }
    // The `╰─ <path> +N -M` summary line renders in both states; ctrl+j only
    // attaches or removes the indented diff rows underneath it.
    renderDiffs(lines, width, diffs, hasCode) {
        ...
        for (const diff of diffs) {
            ...
            if (existing)
                ...
                diffsByPath.set(diff.path, [diff]);
        }
        if (hasCode) {
            this.addPlain(lines, "");
        }
    ...
    renderFileDiff(lines, width, path, edits, showHint) {
...
//# sourceMappingURL=ipython-cell.js.map