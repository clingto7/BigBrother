...
export class BashExecutionComponent extends Container {
    ...
    expanded = false;
    contentContainer;
    constructor(command, ui, excludeFromContext = false, options = {}) {
        super();
        ...
        this.loader = new Loader(ui, (spinner) => theme.fg("muted", spinner), (text) => theme.fg("muted", text), `Running... (${keyText("tui.select.cancel")} to cancel)`);
        ...
        if (this.status === "running") {
            ...
            if (hiddenLineCount > 0) {
                if (this.expanded) {
                    ...
                    statusParts.push(`${theme.fg("muted", `... ${hiddenLineCount} more lines`)} ${expandCollapseHint("app.tools.expand", false)}`);
                }
            }
            if (this.status === "cancelled") {
                ...
                statusParts.push(theme.fg("error", this.errorMessage !== undefined ? `(failed: ${this.errorMessage})` : `(exit ${this.exitCode})`));
            }
