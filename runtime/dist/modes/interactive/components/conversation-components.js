...
import { COMPACTION_OUTCOME_CUSTOM_TYPE, isCompactionOutcomeMessage, isRefinementOutcomeMessage, isSessionSlashCommandMessage, isSessionSlashCommandResultMessage, REFINEMENT_OUTCOME_CUSTOM_TYPE, SESSION_SLASH_COMMAND_CUSTOM_TYPE, SESSION_SLASH_COMMAND_RESULT_CUSTOM_TYPE, } from "../../../core/messages.js";
import { AgentMessageComponent } from "./agent-message.js";
import { AssistantMessageComponent } from "./assistant-message.js";
import { BashExecutionComponent } from "./bash-execution.js";
import { CompactionOutcomeMessageComponent, MalformedCompactionOutcomeMessageComponent, } from "./compaction-outcome-message.js";
...
import { MalformedRefinementOutcomeMessageComponent, RefinementOutcomeMessageComponent, } from "./refinement-outcome-message.js";
...
import { selectLatestToolExpandHint, ToolExecutionComponent, } from "./tool-execution.js";
import { UserMessageComponent } from "./user-message.js";
export function isCompactAgentMessageNeighbor(component) {
    return (component instanceof AgentMessageComponent ||
        component instanceof ToolExecutionComponent ||
        component instanceof IPythonCellComponent ||
        component instanceof BashExecutionComponent);
}
function readUserText(content) {
...
export function buildConversationComponents(messages, options) {
    const components = [];
    ...
    for (const message of messages) {
        if (message.role === "assistant") {
            components.push(new AssistantMessageComponent(message, options.hideThinkingBlock ?? false, options.markdownTheme, options.hiddenThinkingLabel ?? "Thinking...", {
                expanded,
            ...
            for (const content of message.content) {
                ...
                const tool = new ToolExecutionComponent(content.name, content.id, content.arguments, { ...options.toolOptions, includeImageDimensions: false }, options.getToolDefinition(content.name), options.ui, options.cwd);
            ...
            if (!message.display)
                continue;
            components.push(isCompactionOutcomeMessage(message)
            ...
            const hasContent = typeof message.content === "string" ? message.content.length > 0 : message.content.length > 0;
...
//# sourceMappingURL=conversation-components.js.map