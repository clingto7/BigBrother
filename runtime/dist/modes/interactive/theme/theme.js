...
import { bestAnsiColor, blendColor, getDefaultTerminalColors, getTerminalBackgroundKind, isLightColor, onDefaultTerminalColorsChange, rgbTo256, } from "@earendil-works/pi-tui";
...
let validateThemeJson;
let themeValidatorPromise;
export function preloadThemeValidator() {
...
const ADAPTIVE_LIGHT_BG_ACCENT = { r: 0, g: 95, b: 135 };
...
export class Theme {
    ...
    constructor(fgColors, bgColors, mode, options = {}) {
        ...
        this.bgColorValues = new Map(Object.entries(bgColors));
    }
    fg(color, text) {
    ...
    getSelectionBackgroundColor() {
        ...
        if (!terminalBg || (typeof selectedBgValue === "number" && selectedBgValue < 16)) {
            return (str) => this.bg("selectedBg", str);
        }
    ...
    getAdaptiveAccentColor() {
        const ansi = getTerminalBackgroundKind() === "light"
            ? fgAnsi(bestAnsiColor(ADAPTIVE_LIGHT_BG_ACCENT, this.mode), this.mode)
            : "\x1b[36m";
        return (str) => `${ansi}\x1b[1m${str}\x1b[22m\x1b[39m`;
    }
    bold(text) {
...
function getBuiltinThemes() {
    if (!BUILTIN_THEMES) {
        ...
        BUILTIN_THEMES = {
            prime: JSON.parse(fs.readFileSync(primePath, "utf-8")),
 ...
 */
export function preloadCodeHighlighter() {
...
export function getMarkdownTheme() {
    return {
        ...
        math: (text) => theme.fg("mdCode", text),
        mathBlock: (text) => theme.fg("mdCodeBlock", text),
...
export function getEditorTheme() {
    return {
        ...
        autocompleteBackgroundColor: (text) => theme.getPopupBackgroundColor()(text),
