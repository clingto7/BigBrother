...
export function resolveSummaryModel(registry) {
...
export function buildStatusContext(messages, isWorking) {
...
export function parseAgentStatusResponse(text, isWorking) {
    ...
    try {
        const response = await completeSimple(model, {
            ...
            messages: [
                {
                    role: "user",
                    content: [{ type: "text", text: buildStatusContext(messages, isWorking) }],
                    timestamp: Date.now(),
                },
            ],
...
export function agentStatusChanged(previous, next) {
    if (!previous) {
        return true;
    }
    return previous.summary !== next.summary || previous.taskState !== next.taskState;
}
function isSessionWorking(state) {
...
export class DaemonSessionSummarizer {
    ...
    constructor(listSessions, onStatusChanged, 
    // Injectable for tests.
    generate = generateAgentStatus) {
        ...
        try {
            ...
            const result = generated ??
                (!isWorking && (owesIdleVerdict || owesSummary)
                    ? { summary: previous?.summary ?? "", taskState: "needs_input" }
                    : undefined);
            if (!result) {
                return;
            }
            ...
            const taskState = result.taskState ?? (previous?.basedOnMessageCount === messageCount ? previous?.taskState : undefined);
            const status = {
...
//# sourceMappingURL=daemon-session-summarizer.js.map