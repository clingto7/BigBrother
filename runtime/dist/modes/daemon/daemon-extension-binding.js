...
import { isDaemonDialogExtensionUiRequest, } from "./daemon-protocol.js";
/**
 ...
 */
function slimSessionEventForWire(event) {
...
export async function bindActiveSessionState(state, callbacks) {
    ...
    state.runtime.setRebindSession(async () => {
        ...
        callbacks.broadcast(state, {
            ...
            activeSessionId: state.activeSessionId,
            state: callbacks.createConnectionState?.(state) ??
...
function createCommandContextActions(state) {
    return {
...
function createExtensionUIContext(state, broadcast) {
    const emitUiRequest = (method, payload) => {
        const id = randomUUID();
        broadcast(state, {
    ...
    const dialogRequest = (method, payload, opts, fallback, resolveResponse) => {
    ...
    return {
        select: (title, values, opts) => dialogRequest("select", { title, options: values, timeout: opts?.timeout }, opts, undefined, (response) => "cancelled" in response && response.cancelled
            ? undefined
            : "value" in response
                ? response.value
                : undefined),
        confirm: (title, message, opts) => dialogRequest("confirm", { title, message, timeout: opts?.timeout }, opts, false, (response) => "confirmed" in response ? response.confirmed : false),
        input: (title, placeholder, opts) => dialogRequest("input", { title, placeholder, timeout: opts?.timeout }, opts, undefined, (response) => "cancelled" in response && response.cancelled
            ? undefined
            : "value" in response
                ? response.value
                : undefined),
        ...
        setWorkingIndicator: (indicatorOptions) => emitUiRequest("setWorkingIndicator", { options: indicatorOptions }),
        ...
        editor: (title, prefill) => {
            return dialogRequest("editor", { title, prefill }, undefined, undefined, (response) => "cancelled" in response && response.cancelled
                ? undefined
                : "value" in response
                    ? response.value
                    : undefined);
...
function hasExtensionUiClientForMethod(state, method) {
...
//# sourceMappingURL=daemon-extension-binding.js.map