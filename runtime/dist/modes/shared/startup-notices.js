...
export async function gatherStartupNotices(options) {
...
export function formatUpdateAvailableNotice(newVersion) {
    return (`${theme.bold(theme.fg("accent", "Update available:"))} ` +
        `${theme.fg("muted", `v${newVersion}. Run `)}${theme.fg("accent", "/update")}`);
}
export function formatPackageUpdateNotice(packages) {
    const packageList = packages.join(", ");
    return (`${theme.bold(theme.fg("warning", "Package updates available:"))} ` +
        `${theme.fg("muted", `${packageList}. Run `)}${theme.fg("accent", "/update --extensions")}`);
}
export function formatTmuxWarningNotice(message) {
    return theme.fg("warning", `⚠ ${message}`);
}
//# sourceMappingURL=startup-notices.js.map