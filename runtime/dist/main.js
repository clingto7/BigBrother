...
import { ensureInteractiveDaemonRunning, isDaemonSessionSummary, listActiveDaemonSessionSummaries, probeRunningDaemonSessions, StaleDaemonError, shutdownDaemonAndWait, } from "./cli/daemon-launch.js";
...
import { looksLikeSessionPath, resolveSessionPath, SessionSelectorError, SessionSelectorNotFoundError, } from "./cli/session-resolver.js";
import { APP_NAME, expandTildePath, getAgentDir, getSessionDirEnvOverride, VERSION } from "./config.js";
import { mergeAgentSessionRuntimeConfig, mergeAutonomousConfig, } from "./core/agent-session-config.js";
...
import { findInitialModel, resolveCliModel, resolveModelScope } from "./core/model-resolver.js";
...
import { DAEMON_WORKER_ACTIVE_SESSION_ID_ENV, isDaemonWorkerProcess, requireDaemonWorkerAuthenticationToken, waitForDaemonWorkerStartupGate, } from "./modes/daemon/daemon-worker-protocol.js";
import { ClientPromptStashStore, createInteractiveModeLocalSessionHost, createInteractiveModeUiServicesFromServices, DaemonAgentConnection, DaemonCapabilityUnavailableError, DaemonClient, defaultDaemonSocketPath, InProcessAgentConnection, InteractiveMode, normalizeSocketPath, resolveAttachModelFallbackMessage, runAcpMode, runAcpModeWithConnection, runAgentsViewMode, runDaemonMode, runDaemonSupervisorMode, runPrintMode, runPrintModeWithConnection, runRpcMode, runRpcModeWithConnection, } from "./modes/index.js";
...
export function shouldRejectNonInteractiveAttach(attachAgent, appMode) {
    return attachAgent !== undefined && appMode !== "interactive";
}
export function shouldRejectNonInteractiveBareResume(resume, appMode) {
...
export function isClientOwnedDaemonSession(appMode, noSession) {
    return appMode !== "acp" || noSession === true;
}
// `prime-agent agents` opens the agents view directly.
export function parseAgentsViewCommand(args) {
    if (args[0] === "agents") {
...
export function shouldUseDaemonInteractive(options) {
    return (options.appMode === "interactive" &&
        !options.startupBenchmark &&
        !options.noSession &&
        options.listModels === undefined);
}
export function shouldUseDaemonClient(options) {
    return (options.appMode !== "daemon" && !options.startupBenchmark && !options.help && options.listModels === undefined);
}
export function shouldUseDaemonClientRuntime(options) {
    return shouldUseDaemonClient(options) && !options.ownedSessionWorker && !options.hasProcessLocalExtensionFactories;
}
export function shouldEnsureInteractiveDaemonForStartup(useDaemonInteractive, attachAgent) {
    return useDaemonInteractive && attachAgent === undefined;
}
export function shouldOpenAgentsViewForDaemonInteractive(options) {
    ...
    return (options.useDaemonInteractive &&
...
export function shouldUseEphemeralSessionManagerForDaemonInteractive(options) {
    return (!options.hasActiveDaemonSession &&
        (options.resume === undefined || options.resume === true) &&
        !options.continue &&
        !options.fork);
}
export function shouldEnsureDaemonBeforeActiveSessionLookup(options) {
    return (options.useDaemonInteractive &&
        options.resumeSelector !== undefined &&
        (options.explicitAttach || !looksLikeSessionPath(options.resumeSelector)));
}
async function findActiveDaemonSessionSummaryForInteractiveStartup(socketPath, selector, options = {}) {
    try {
...
const STARTUP_SESSION_LOSS_COPY = {
    busyDetail(count) {
    ...
    unlistableDetail: "A background service from a different Prime Agent version is running and its sessions could not be listed. Stopping it may terminate active sessions.",
...
async function takeOverStaleDaemonOrExit(socketPath) {
    ...
    if (!(await shutdownDaemonAndWait(socketPath))) {
        console.error(chalk.red(`Could not stop the background service on ${socketPath}. Run "prime-agent shutdown" and retry.`));
        process.exit(1);
    }
...
async function awaitDaemonReady(daemonReady) {
    if (!daemonReady) {
...
function getResumeSelector(parsed) {
    return typeof parsed.resume === "string" ? parsed.resume : undefined;
}
export async function createSessionManager(parsed, cwd, sessionDir) {
...
function buildSessionOptions(config, scopedModels, hasExistingSession, modelRegistry, settingsManager) {
    const options = {};
    const diagnostics = [];
...
function runtimeAutonomousConfigFromArgs(parsed) {
    const hasAutonomousOptions = parsed.autonomous === true ||
    ...
    const hasGateOptions = parsed.autonomousGates !== undefined ||
        parsed.autonomousGateRetries !== undefined ||
        parsed.autonomousGateTimeoutMs !== undefined;
    return {
        enabled: true,
...
function runtimeConfigFromArgs(parsed, cwd, agentDir, sessionDir, appMode, telemetryDisabled) {
    return {
        cwd,
        agentDir,
        sessionDir,
...
export function daemonServerDefaultSessionConfig(config) {
    return { ...config, initialGoal: undefined };
}
export function resolveRuntimeSessionOptions(sessionOptions, runtimeSessionOptions) {
    return {
        ...
        autonomous: (runtimeSessionOptions?.rlmDepth ?? 0) > 0
...
async function prepareRuntimeServices(options) {
    ...
    const { options: sessionOptions, cliThinkingFromModel, diagnostics: sessionOptionDiagnostics, } = buildSessionOptions(config, scopedModels, sessionManager.buildSessionContext().messages.length > 0, modelRegistry, settingsManager);
...
function getDaemonSummaryActiveSessionId(summary) {
    return summary.activeSessionId ?? summary.id;
}
function isUnknownActiveSessionError(message) {
    return message.startsWith("Unknown active session:");
}
async function findActiveDaemonSessionSummary(socketPath, selector) {
...
function createSessionManagerForActiveDaemonSummary(summary, fallbackCwd) {
...
function getInteractiveDaemonSessionPath(parsed, sessionManager) {
    if (!parsed.resume && !parsed.continue && !parsed.fork) {
        return undefined;
    }
    return sessionManager.getSessionFile();
}
export function findActiveDaemonSessionSummaryForSessionFile(summaries, sessionPath) {
...
async function createDaemonClientConnection(options) {
    ...
    try {
        ...
        if (options.sessionPath && !options.clientOwned) {
            const activeSummary = findActiveDaemonSessionSummaryForSessionFile(await listActiveDaemonSessionSummaries(client), options.sessionPath);
...
async function findAttachedDaemonSessionSummary(client, activeSessionId) {
...
export async function main(args, options) {
    ...
    const sessionDir = (parsed.sessionDir ? expandTildePath(parsed.sessionDir) : undefined) ??
    ...
    let activeDaemonSessionSummary;
    if (shouldLookupDaemonActiveSession && resumeSelector) {
        try {
            activeDaemonSessionSummary = await findActiveDaemonSessionSummaryForInteractiveStartup(daemonSocketPath, resumeSelector, { fallbackOnError: !publicCommand.attachAgent });
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
    ...
    const telemetrySettingsManager = sessionManager.getCwd() === cwd
    ...
    const defaultSessionConfig = runtimeConfigFromArgs(parsed, sessionManager.getCwd(), agentDir, sessionDir, appMode, telemetryDisabled);
    ...
    const createRuntime = async ({ cwd, agentDir, sessionManager, sessionStartEvent, sessionConfig, sessionOptions: runtimeSessionOptions, }) => {
    ...
    if (useDaemonInteractive) {
        ...
        const launchAgentsView = async (initialSession, initialScopeKey) => {
            await runAgentsViewMode({
                ...
                createUiServicesForSession: async (summary) => {
                    const attachedSessionManager = createSessionManagerForActiveDaemonSummary(summary, sessionManager.getCwd());
        ...
        if (shouldOpenAgentsViewForDaemonInteractive({
        ...
        const isFreshDefaultSession = !activeDaemonSessionSummary && !getInteractiveDaemonSessionPath(parsed, sessionManager);
        ...
        const initialScopeKey = interactiveResult.type === "scoped_agents_view"
            ? {
